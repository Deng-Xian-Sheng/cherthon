...
+ (id)getFactorySettingsForFileType: (NSString *)filetype
{
    ...
    if ([filetype isEqualToString: @"Herthon Script"]) {
        curdefault = &fsdefault_py;
    } else if ([filetype isEqualToString: @"Herthon GUI Script"]) {
        curdefault = &fsdefault_pyw;
    } else if ([filetype isEqualToString: @"Herthon Bytecode Document"]) {
        curdefault = &fsdefault_pyc;
    } else {
        NSLog(@"Funny File Type: %@\n", filetype);
        curdefault = &fsdefault_py;
        filetype = @"Herthon Script";
    }
    if (!*curdefault) {
