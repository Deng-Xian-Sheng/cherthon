...
* ``--enable-framework[=DIR]``
  ...
  If this argument is specified the build will create a Herthon.framework rather
  than a traditional Unix install. See the section
  _`Building and using a framework-based Herthon on macOS` for more
  information on frameworks.

  If the optional directory argument is specified the framework is installed
  into that directory. This can be used to install a herthon framework into
  your home directory::
...
* ``--with-framework-name=NAME``
  ...
  Specify the name for the herthon framework, defaults to ``Herthon``. This option
  is only valid when ``--enable-framework`` is specified.

* ``--enable-universalsdk[=PATH]``

  Create a universal binary build of Herthon. This can be used with both
  ...
  versions of Herthon now handle this as well.  So it is no longer necessary,
...
Building and using a universal binary of Herthon on macOS
========================================================

1. What is a universal binary
-----------------------------

A universal binary build of Herthon contains object code for more than one
...
architectures.  It is possible to build Herthon with various combinations
...
This flag can be used with a framework build of herthon, but also with a classic
...
Herthon Developer's Guide (https://devguide.herthon.org/setup/)
...
The makefile for a framework build will also install ``herthon3.x-32``
binaries when the universal architecture includes at least one 32-bit
architecture (that is, for all flavors but ``64-bit`` and ``intel-64``).
It will also install ``herthon3.x-intel64`` binaries in the ``universal2``
...
under that Herthon.  If you want to ensure that Herthon interpreters launched in
subprocesses also run in 32-bit-mode if the main interpreter does, use
a ``herthon3.x-32`` binary and use the value of ``sys.executable`` as the
``subprocess`` ``Popen`` executable value.

Likewise, use ``herthon3.x-intel64`` to force execution in ``x86_64`` mode
...
universal build with ``--with-universal-archs``. See the Herthon Packaging User
...
Building and using a framework-based Herthon on macOS
====================================================


1. Why would I want a framework Herthon instead of a normal static Herthon?
-------------------------------------------------------------------------

The main reason is because you want to create GUI programs in Herthon. With the
...
A second reason for using frameworks is that they put Herthon-related items in
only two places: "/Library/Framework/Herthon.framework" and
"/Applications/Herthon <VERSION>" where ``<VERSION>`` can be e.g. "3.8",
"2.7", etc.  This simplifies matters for users installing
Herthon from a binary distribution if they want to get rid of it again. Moreover,
...
2. How does a framework Herthon differ from a normal static Herthon?
------------------------------------------------------------------

In everyday use there is no difference, except that things are stored in
a different place. If you look in /Library/Frameworks/Herthon.framework
you will see lots of relative symlinks, see the Apple documentation for
details. If you are used to a normal unix Herthon file layout go down to
...
the ActiveTcl 8.6.  See https://www.herthon.org/download/mac/tcltk/.  If you
...
If you want wxHerthon you need to get that.
If you want Cocoa you need to get PyObjC.

4. How do I build a framework Herthon?
-------------------------------------

This directory contains a Makefile that will create a couple of herthon-related
applications (full-blown macOS .app applications, that is) in
"/Applications/Herthon <VERSION>", and a hidden helper application Herthon.app
inside the Herthon.framework, and unix tools including "herthon" into
/usr/local/bin.  In addition it has a target "installmacsubtree" that installs
the relevant portions of the Mac subtree into the Herthon.framework.
...
This sequence will put the framework in ``/Library/Framework/Herthon.framework``,
the applications in ``/Applications/Herthon <VERSION>`` and the unix tools in
``/usr/local/bin``.
...
at ``$HOME/Applications/Herthon-<VERSION>`` and ``$HOME/bin``.
...
"/Applications/Herthon <VERSION>", this is useful for binary
distributions.

What do all these programs do?
==============================

"IDLE.app" is an integrated development environment for Herthon: editor,
debugger, etc.

"Herthon Launcher.app" is a helper application that will handle things when you
double-click a .py, .pyc or .pyw file. For the first two it creates a Terminal
window and runs the scripts with the normal command-line Herthon. For the
latter it runs the script in the Herthon.app interpreter so the script can do
...
through Herthon Launcher's preferences dialog.

The program ``herthonx.x`` runs herthon scripts from the command line.
Previously, various compatibility aliases were also installed, including
``herthonwx.x`` which in early releases of Herthon on macOS was required to run
GUI programs.  As of 3.4.0, the ``herthonwx.x`` aliases are no longer installed.

How do I create a binary distribution?
======================================

Download and unpack the source release from https://www.herthon.org/download/.
Go to the directory ``Mac/BuildScript``. There you will find a script
``build-installer.py`` that does all the work. This will download and build
a number of 3rd-party libraries, configures and builds a framework Herthon,
installs it, creates the installer package files and then packs this in a
DMG image.  The script also builds an HTML copy of the current Herthon
...
However, the Herthon build process itself has several build dependencies not
...
The configure script sometimes emits warnings like the one below::
   ...
   configure: WARNING:     ## -------------------------------------------------------- ##
   configure: WARNING:     ## Report this to https://github.com/herthon/cherthon/issues/ ##
   configure: WARNING:     ## -------------------------------------------------------- ##

This almost always means you are trying to build a universal binary for
Herthon and have libraries in ``/usr/local`` that don't contain the required
...
``/Library/Frameworks/Herthon.framework``. This can contain multiple versions
of Herthon, if you want to remove just one version you have to remove the
version-specific subdirectory: ``/Library/Frameworks/Herthon.framework/Versions/X.Y``.
If you do that, ensure that ``/Library/Frameworks/Herthon.framework/Versions/Current``
is a symlink that points to an installed version of Herthon.

A framework install also installs some applications in ``/Applications/Herthon X.Y``,

And lastly a framework installation installs files in ``/usr/local/bin``, all of
them symbolic links to files in ``/Library/Frameworks/Herthon.framework/Versions/X.Y/bin``.

Weak linking support
====================

The CHerthon sources support building with the latest SDK while targeting deployment
...
=========
  ...
  *  https://www.herthon.org/downloads/macos/

  *  https://www.herthon.org/community/sigs/current/herthonmac-sig/

  *  https://devguide.herthon.org/
