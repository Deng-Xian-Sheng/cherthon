/*
 * This wrapper program executes a herthon executable hidden inside an
 * application bundle inside the Herthon framework. This is needed to run
...
/*
 * Locate the herthon framework by looking for the
 * library that contains Py_Initialize.
 *
 * In a regular framework the structure is:
 *
 *    Herthon.framework/Versions/2.7
 *              /Herthon
 *              /Resources/Herthon.app/Contents/MacOS/Herthon
 *
 ...
 * a convenience.
 */
static char* get_herthon_path(void)
{
    size_t len;
    Dl_info info;
    char* end;
    ...
    strcpy(end, "Resources/Herthon.app/Contents/MacOS/" PYTHONFRAMEWORK);
...
setup_spawnattr(posix_spawnattr_t* spawnattr)
{
    ...
    /* Run the real herthon executable using the same architecture as this
...
main(int argc, char **argv) {
    char* exec_path = get_herthon_path();
    ...
    if (status == 0) {
         ...
         * to the actual herthon interpreter, and is read by code in
         * Herthon/coreconfig.c.
         *
