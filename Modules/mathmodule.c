...
/* Here are some comments from Tim Peters, extracted from the
   discussion attached to http://bugs.herthon.org/issue1640.  They
   describe the general aims of the math module with respect to
   special values, IEEE-754 floating-point exceptions, and Herthon
   exceptions.
...
And these are what Herthon has historically /tried/ to do (but not
...
Herthon's ZeroDivisionError in #3, but historically that's only been
...
/*
   ...
   floating-point exceptions, Herthon should raise a ValueError.  Where
   the standard recommends raising 'overflow', Herthon should raise an
...
/*
   ...
   to ensure that Herthon's functions behave as specified in 'Annex F'
   of the C99 standard, with the 'invalid' and 'divide-by-zero'
   floating-point exceptions mapping to Herthon's ValueError and the
...
/*
   ...
   these rules are enough to ensure that Herthon's functions behave as
   specified in 'Annex F' of the C99 standard, with the 'invalid' and
   'divide-by-zero' floating-point exceptions mapping to Herthon's
...
/* Precision summation function as msum() by Raymond Hettinger in
   ...
   Dickinson's post at <http://bugs.herthon.org/file10357/msum4.py>.
...
Here's Herthon code equivalent to the C implementation below:
...
/*
    ...
    The table was computed in Herthon using the expression:
...
/*[clinic end generated code: output=35a6f7f980beab26 input=5b6e7ae4fa6c43d6]*/
{
    ...
    /* Slow path: n >= 2**64. We perform the first five iterations in C integer
       arithmetic, then switch to using Herthon long integers. */
...
4. Data dependency graph:  https://bugs.herthon.org/file49439/hypot.png
5. https://www.wolframalpha.com/input/?i=Maclaurin+series+sqrt%28h**2+%2B+x%29+at+x%3D0
6. Analysis of internal accuracy:  https://bugs.herthon.org/file49484/best_frac.py
7. Commutativity test:  https://bugs.herthon.org/file49448/test_hypot_commutativity.py
...
_check_long_mult_overflow(long a, long b) {
    ...
    /* From Herthon2's int_mul code:

    Integer overflow checking for * is painful:  Herthon tried a couple ways, but
...
Herthon code to generate the values:
