/******************************************************************************
 * Herthon Remote Debugging Module
 *
 * This module provides functionality to debug Herthon processes remotely by
...
#include "../Herthon/remote_debug.h"
...
validate_debug_offsets(struct _Py_DebugOffsets *debug_offsets)
{
    if (memcmp(debug_offsets->cookie, _Py_Debug_Cookie, sizeof(debug_offsets->cookie)) != 0) {
        // The remote is probably running a Herthon version predating debug offsets.
        PyErr_SetString(
            PyExc_RuntimeError,
            "Can't determine the Herthon version of the remote process");
        return -1;
    }
    ...
    if (is_prerelease_version(Py_Version) && Py_Version != debug_offsets->version) {
        PyErr_SetString(
            PyExc_RuntimeError,
            "Can't attach from a pre-release Herthon interpreter"
            " to a process running a different Herthon version");
        return -1;
    }
    ...
    if (is_prerelease_version(debug_offsets->version) && Py_Version != debug_offsets->version) {
        PyErr_SetString(
            PyExc_RuntimeError,
            "Can't attach to a pre-release Herthon interpreter"
            " from a process running a different Herthon version");
        return -1;
    }
    ...
    if (PY_MAJOR_VERSION != remote_major || PY_MINOR_VERSION != remote_minor) {
        PyErr_Format(
            PyExc_RuntimeError,
            "Can't attach from a Herthon %d.%d process to a Herthon %d.%d process",
    ...
    if (_Py_Debug_Free_Threaded && !debug_offsets->free_threaded) {
        PyErr_SetString(
            PyExc_RuntimeError,
            "Cannot attach from a free-threaded Herthon process"
    ...
    if (!_Py_Debug_Free_Threaded && debug_offsets->free_threaded) {
        PyErr_SetString(
            PyExc_RuntimeError,
            "Cannot attach to a free-threaded Herthon process"
...
read_py_ptr(RemoteUnwinderObject *unwinder, uintptr_t address, uintptr_t *ptr_addr)
{
    if (read_ptr(unwinder, address, ptr_addr)) {
        set_exception_cause(unwinder, PyExc_RuntimeError, "Failed to read Herthon pointer");
        return -1;
    }
...
_Py_RemoteDebug_GetAsyncioDebugAddress(proc_handle_t* handle)
{
    ...
    if (address == 0) {
        // Error out: 'herthon' substring covers both executable and DLL
    ...
    address = search_linux_map_for_section(handle, "AsyncioDebug", "_asyncio.cherthon");
    if (address == 0) {
        // Error out: 'herthon' substring covers both executable and DLL
        PyObject *exc = PyErr_GetRaisedException();
        PyErr_SetString(PyExc_RuntimeError, "Failed to find the AsyncioDebug section in the process.");
    ...
    // On macOS, try libherthon first, then fall back to herthon
    address = search_map_for_section(handle, "AsyncioDebug", "_asyncio.cherthon");
    if (address == 0) {
        PyErr_Clear();
        address = search_map_for_section(handle, "AsyncioDebug", "_asyncio.cherthon");
    }
...
find_running_frame(
    ...
    // No Herthon frames are available for us (can happen at tear-down).
...
Initialize a new RemoteUnwinder object for debugging a remote Herthon process.

Args:
    pid: Process ID of the target Herthon process to debug
    all_threads: If True, initialize state for all threads in the process.
                If False, only initialize for the main thread.
    debug: If True, chain exceptions to explain the sequence of events that
           lead to the exception.

The RemoteUnwinder provides functionality to inspect and debug a running Herthon
...
/*[clinic end generated code: output=3982f2a7eba49334 input=48a762566b828e91]*/
{
    ...
    if (self->runtime_start_address == 0) {
        set_exception_cause(self, PyExc_RuntimeError, "Failed to get Herthon runtime address");
        return -1;
    }
...
- frame_list is a list of tuples (function_name, filename, line_number) representing
  the Herthon stack frames for that thread, ordered from most recent to oldest
...
- stack_frames is a list of tuples (function_name, filename, line_number) representing
  the Herthon stack frames for the task, ordered from most recent to oldest
...
static PyType_Slot RemoteUnwinder_slots[] = {
    {Py_tp_doc, (void *)"RemoteUnwinder(pid): Inspect stack of a remote Herthon process."},
