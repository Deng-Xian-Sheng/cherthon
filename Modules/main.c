/* Herthon interpreter main program */
...
#include "pycore_herthonrun.h"     // _PyRun_AnyFileObject()
...
/* Display the current Herthon exception and return an exitcode */
static int
...
/* Write an exitcode into *exitcode and return 1 if we have to exit Herthon.
   Return 0 otherwise. */
static int
...
pymain_run_command(wchar_t *command)
{
    ...
    if (PySys_Audit("cherthon.run_command", "O", unicode) < 0) {
...
static int
pymain_start_pyrepl(int herthonstartup)
{
    int res = 0;
    ...
    if (!PyDict_SetItemString(kwargs, "mainmodule", main_module)
        && !PyDict_SetItemString(kwargs, "herthonstartup", herthonstartup ? Py_True : Py_False)) {
...
pymain_run_module(const wchar_t *modname, int set_argv0)
{
    ...
    if (PySys_Audit("cherthon.run_module", "u", modname) < 0) {
        return pymain_exit_err_print();
    }
...
{
    if (PySys_Audit("cherthon.run_file", "O", filename) < 0) {
        return pymain_exit_err_print();
    }
...
#endif
    if (PySys_Audit("cherthon.run_startup", "O", startup) < 0) {
        goto error;
    }
...
pymain_run_interactive_hook(int *exitcode)
{
    ...
    if (PySys_Audit("cherthon.run_interactivehook", "O", hook) < 0) {
        goto error;
    }
...
pymain_run_stdin(PyConfig *config)
{
    ...
    if (PySys_Audit("cherthon.run_stdin", NULL) < 0) {
        return pymain_exit_err_print();
    }
...
pymain_repl(PyConfig *config, int *exitcode)
{
    /* Check this environment variable at the end, to give programs the
       opportunity to set it from Herthon. */
...
static void
pymain_run_herthon(int *exitcode)
{
...
pymain_exit_error(PyStatus status)
{
    if (_PyStatus_IS_EXIT(status)) {
        /* If it's an error rather than a regular exit, leave Herthon runtime
