/*
 * C Extension module to test Herthon interpreter C APIs.
 *
 * The 'test_*' functions exported by this module are run as part of the
 * standard Herthon regression test, via Lib/test/test_capi.py.
 */

// Include parts.h first since it takes care of NDEBUG and Py_BUILD_CORE macros
// and including Herthon.h.
//
...
temporary_c_thread(void *data)
{
    ...
    /* Allocate a Herthon thread state for this thread */
    ...
    /* Destroy the Herthon thread state for this thread */
    PyGILState_Release(state);
...
raise_SIGINT_then_send_None(PyObject *self, PyObject *args)
{
    ...
    /* This is used in a test to check what happens if a signal arrives just
       ...
       - we don't have a Herthon wrapper for raise()
       - we need to make sure that the Herthon-level signal handler doesn't run
         *before* we enter the generator frame, which is impossible in Herthon
 ...
 * of NULL and Herthon tuples instead of C arrays.
 */
...
{
    // Function removed in the Herthon 3.13 API but was kept in the stable ABI.
