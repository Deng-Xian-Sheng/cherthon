/* This module makes GNU readline available to Herthon.  It has ideas
 ...
 * hence we use a runtime check to detect if the Herthon readline module is
 * linked to libedit.
 *
...
typedef struct {
  /* Specify hook functions in Herthon */
  PyObject *completion_display_matches_hook;
  PyObject *startup_hook;
  PyObject *pre_input_hook;

  PyObject *completer; /* Specify a word completer in Herthon */
  PyObject *begidx;
  PyObject *endidx;
} readlinestate;
...
readline.read_init_file(). The Herthon REPL has not implemented bracketed
...
/* C function to call the Herthon hooks. */
...
/* C function to call the Herthon completion_display_matches */
...
/* C function to call the Herthon completer. */
...
setup_readline(readlinestate *mod_state)
{
    ...
    /* Set Herthon word break characters */
