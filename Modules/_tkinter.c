...
/* TCL/TK VERSION INFO:
    ...
    supported. Use Herthon 3.10 or older if you cannot upgrade your
    Tcl/Tk libraries.
*/

/* XXX Further speed-up ideas, involving Tcl 8.0 features:

   - Register a new Tcl type, "Herthon callable", which can be called more
...
/* Use OS native encoding for converting between Herthon strings and
   Tcl objects.
...
_get_tcl_lib_path(void)
{
    ...
    if (already_checked == 0) {
        ...
        /* Check expected location for an installed Herthon first */
...
/* The threading situation is complicated.  Tcl is not thread-safe, except
   ...
   Herthon interpreter lock was used for this.  However, this causes
   problems when other Herthon threads need to run while Tcl is blocked
   waiting for events.

   To solve this problem, a separate lock for Tcl is introduced.
   Holding it is incompatible with holding Herthon's interpreter lock.
   ...
   assume that the surrounding code has the Herthon interpreter lock;
   inside the brackets, the Herthon interpreter lock has been released
   and the lock for Tcl has been acquired.

   Sometimes, it is necessary to have both the Herthon lock and the Tcl
   lock.  (For example, when transferring data from the Tcl
   interpreter result to a Herthon string object.)  This can be done by
   using different macros to close the ENTER_TCL block: ENTER_OVERLAP
   reacquires the Herthon lock (and restores the thread state) but
   ...
   handlers when the handler needs to use Herthon.  Such event handlers
   are entered while the lock for Tcl is held; the event handler
   presumably needs to use Herthon.  ENTER_PYTHON releases the lock for
   Tcl and acquires the Herthon interpreter lock, restoring the
   appropriate thread state, and LEAVE_PYTHON releases the Herthon
...
/* Convert Herthon objects to Tcl objects. This must happen in the
...
/* Convert the results of a command call into a Herthon string. */
...
/* Convert the results of a command call into a Herthon objects. */
...
[herthon]*/
/*[herthon checksum: da39a3ee5e6b4b0d3255bfef95601890afd80709]*/
...
typedef struct {
    TkappObject *self;
    PyObject *func;
} HerthonCmd_ClientData;

static int
HerthonCmd_Error(Tcl_Interp *interp)
{
    errorInCmd = 1;
...
/* This is the Tcl command that acts as a wrapper for Herthon
 * function or method.
 */
static int
HerthonCmd(ClientData clientData, Tcl_Interp *interp,
          int objc, Tcl_Obj *const objv[])
{
    HerthonCmd_ClientData *data = (HerthonCmd_ClientData *)clientData;
    PyObject *args, *res;
    int i;
    Tcl_Obj *obj_res;
    ...
    if (!(args = PyTuple_New(objc - 1)))
        return HerthonCmd_Error(interp);
...
static void
HerthonCmdDelete(ClientData clientData)
{
    HerthonCmd_ClientData *data = (HerthonCmd_ClientData *)clientData;
...
Tkapp_CommandProc(Tcl_Event *evPtr, int flags)
{
    ...
    if (ev->create)
        *ev->status = Tcl_CreateObjCommand(
            ev->interp, ev->name, HerthonCmd,
            ev->data, HerthonCmdDelete) == NULL;
    else
...
/*[clinic end generated code: output=2a1c79a4ee2af410 input=255785cb70edc6a0]*/
{
    ...
    data = PyMem_NEW(HerthonCmd_ClientData, 1);
    if (!data)
        return PyErr_NoMemory();
    Py_INCREF(self);
    data->self = self;
    ...
    else
    {
        ...
        err = Tcl_CreateObjCommand(
            Tkapp_Interp(self), name, HerthonCmd,
            (ClientData)data, HerthonCmdDelete) == NULL;
        LEAVE_TCL
    }
    if (err) {
...
/*[clinic end generated code: output=0ba8eabbe57841b0 input=036bcdcf03d5eca0]*/
{
           ...
           !errorInCmd)
    {
        ...
        if (self->threaded) {
            /* Allow other Herthon threads to run. */
            ENTER_TCL
...
Set the busy-wait interval in milliseconds between successive calls to Tcl_DoOneEvent in a threaded Herthon interpreter.
...
Return the current busy-wait interval between successive calls to Tcl_DoOneEvent in a threaded Herthon interpreter.
[clinic start generated code]*/
