...
/*[herthon end generated code: output=da39a3ee5e6b4b0d input=b5689d14466b6823]*/
#endif
...
/*
   ...
   this doesn't make much sense in Herthon anyway) nor do we support
   ...
   only oddity is that the thread executing the Herthon signal handler
...
// State shared by all Herthon interpreters
...
{
    // See https://github.com/herthon/cherthon/pull/102399
...
trip_signal(int sig_num)
{
    ...
    /* And then write to the wakeup fd *after* setting all the globals and
       ...
       - trip_signal sets the flags to request the Herthon-level signal handler
         be run
...
signal_handler(int sig_num)
{
    ...
    /* To avoid infinite recursion, this signal remains
       ...
       to the Herthon handler... */
    if (sig_num != SIGCHLD)
#endif
    /* If the handler was not set up with sigaction, reinstall it.  See
     * Herthon/pylifecycle.c for the implementation of PyOS_setsig which
...
signal_get_set_handlers(signal_state_t *state, PyObject *mod_dict)
{
    ...
    // Install Herthon SIGINT handler which raises KeyboardInterrupt
...
PyErr_CheckSignals(void)
{
    ...
    /* Opportunistically check if the GC is scheduled to run and run it
       ...
       Herthon code to ensure signals are handled. Checking for the GC here
...
/* Declared in cherthon/pyerrors.h */
int
_PyErr_CheckSignalsTstate(PyThreadState *tstate)
{
    ...
    for (int i = 1; i < Py_NSIG; i++) {
         ...
         * was called doesn't guarantee that there is still a Herthon
        ...
        if (func == NULL || func == Py_None ||
            compare_handler(func, state->ignore_handler) ||
            compare_handler(func, state->default_handler)) {
            /* No Herthon signal handler due to aforementioned race condition.
...
/* Simulate the effect of a signal arriving. The next time PyErr_CheckSignals
   is called,  the corresponding Herthon signal handler will be raised.
...
signal_install_handlers(void)
{
    ...
    // Import _signal to install the Herthon SIGINT handler
...
/* Returns a manual-reset event which gets tripped whenever
   ...
   Herthon.h does not include windows.h so we do cannot use HANDLE
