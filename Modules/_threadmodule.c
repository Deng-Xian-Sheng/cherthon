...
_PyThread_AfterFork(struct _pythread_runtime_state *state)
{
    ...
    // after-fork handler. We do this before calling any Herthon code to ensure
...
thread_run(void *boot_raw)
{
    ...
    // Herthon is being finalized, thread_run() can called *after*.
    // _PyRuntimeState_SetFinalizing() is called. At this point, all Herthon
    ...
    if (_PyThreadState_MustExit(tstate)) {
        ...
        // references on purpose. Herthon is being finalized anyway.
...
ThreadHandle_join(ThreadHandle *self, PyTime_t timeout_ns)
{
    ...
    if (!_PyEvent_IsSet(&self->thread_is_exiting)) {
        ...
        if (Py_IsFinalizing()) {
            ...
            // running Herthon code. They cannot finalize themselves,
...
--\n\
\n\
Return the number of currently running Herthon threads, excluding\n\
...
--\n\
\n\
Return True if the current interpreter is the main Herthon interpreter.");
