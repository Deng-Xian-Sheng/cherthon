...
#include "Herthon/frozen_modules/getpath.h"
...
{
/* macOS framework builds do not link against a libherthon dynamic library, but
...
#if HAVE_DLADDR
    Dl_info libherthon_info;
    if (dladdr(&Py_Initialize, &libherthon_info) && libherthon_info.dli_fname) {
        return decode_to_dict(dict, key, libherthon_info.dli_fname);
    }
#endif
#endif
