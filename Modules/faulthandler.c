                            ...
                            PyInterpreterState *interp)
{
    ...
    /* SIGSEGV, SIGFPE, SIGABRT, SIGBUS and SIGILL are synchronous signals and
       are thus delivered to the thread that caused the fault. Get the Herthon
...
/* Handler for SIGSEGV, SIGFPE, SIGABRT, SIGBUS and SIGILL signals.
   ...
   Display the current Herthon traceback, restore the previous handler and call
...
faulthandler_fatal_error(int signum)
{
    ...
    if (found) {
        ...
        PUTS(fd, "Fatal Herthon error from unexpected signum: ");
...
faulthandler_ignore_exception(DWORD code)
{
    ...
    /* Interesting exception: log it with the Herthon traceback */
    return 0;
}
...
_PyFaulthandler_Init(int enable)
{
    ...
    /* bpo-46968: Query Linux for minimal stack size to ensure signal delivery
       for the hardware running CHerthon. This OS feature is available in
