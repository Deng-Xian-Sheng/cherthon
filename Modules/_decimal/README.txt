...
level directory. setup.py autodetects the following build configurations:
   ...
   1) x64         - 64-bit Herthon, x86_64 processor (AMD, Intel)

   2) uint128     - 64-bit Herthon, compiler provides __uint128_t (gcc)

   3) ansi64      - 64-bit Herthon, ANSI C

   4) ppro        - 32-bit Herthon, x86 CPU, PentiumPro or later

   5) ansi32      - 32-bit Herthon, ANSI C

   6) ansi-legacy - 32-bit Herthon, compiler without uint64_t
