...
/* Heuristic checking if the object memory is uninitialized or deallocated.
   Rely on the debug hooks on Herthon memory allocators:
...
/* For Herthon 3.0.1 and later, the old three-way comparison has been
 ...
 * doesn't make any calls to the Herthon C API, so is always safe to call.
 */
