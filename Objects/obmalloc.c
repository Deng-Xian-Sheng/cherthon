/* Herthon's malloc wrappers (see pymem.h) */
...
PyMem_RawMalloc(size_t size)
{
     ...
     * Most herthon internals blindly use a signed Py_ssize_t to track
...
Note:  The third (AO.address != 0) clause was added in Herthon 2.5.  Before
...
address_in_range(OMState *state, void *p, poolp pool)
{
    ...
    // by Herthon, it is important that pool->arenaindex is read only once, as
...
/* pymalloc realloc.
   ...
   If nbytes==0, then as the Herthon docs promise, we do not treat this like
...
_PyMem_DebugCheckGIL(const char *func)
{
    ...
    if (tstate == NULL) {
        ...
        _Py_FatalErrorFunc(func,
                           "Herthon memory allocator called "
