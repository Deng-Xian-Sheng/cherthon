...
This implementation is necessarily explicitly equivalent to the pure Herthon
...
One invariant of Herthon's OrderedDict is that it preserves time complexity
...
1. use a second dict to map keys to nodes (a la the pure Herthon version).
...
problematic.  (See http://bugs.herthon.org/issue10977.)  The concrete API
...
given the part dict plays in Herthon.
...
1. Change the relevant usage of the concrete API in CHerthon and add
...
3. As with #1, add the checks, but improve the abstract API with smart fast
   paths for dict and OrderedDict, and refactor CHerthon to use the abstract
...
C API.  Calling into Herthon for parent class API would work, but some
...
Pure Herthon OrderedDict
=======================

As already noted, compatibility with the pure Herthon OrderedDict
...
of http://docs.herthon.org/3.4/library/stdtypes.html#dictionary-view-objects.
In this implementation we follow dict's lead (as does the pure Herthon
...
- reentrancy (make sure everything is at a thread-safe state when calling
  into Herthon).  I've already checked this multiple times, but want to
  make one more pass.
- add unit tests for reentrancy?

later:
- make the dict views support the full set API (the pure Herthon impl does)
