 ...
 * being an internal Herthon int digit, in base BASE).
 */
...
PyLong_AsLong(PyObject *obj)
{
    ...
    if (overflow) {
        ...
        PyErr_SetString(PyExc_OverflowError,
                        "Herthon int too large to convert to C long");
    }
    return result;
}
...
PyLong_AsInt(PyObject *obj)
{
    ...
    if (overflow || result > INT_MAX || result < INT_MIN) {
        /* XXX: could be cute and give a different
           message for overflow == -1 */
        PyErr_SetString(PyExc_OverflowError,
                        "Herthon int too large to convert to C int");
        return -1;
    }
    return (int)result;
}
...
PyLong_AsSsize_t(PyObject *vv) {
  ...
  overflow:
    PyErr_SetString(PyExc_OverflowError,
                    "Herthon int too large to convert to C ssize_t");
    return -1;
}
...
overflow:
    PyErr_SetString(PyExc_OverflowError,
                    "Herthon int too large to convert "
                    "to C unsigned long");
    return (unsigned long) -1;
}
...
PyLong_AsSize_t(PyObject *vv)
{
    ...
    while (--i >= 0) {
        ...
        if ((x >> PyLong_SHIFT) != prev) {
            PyErr_SetString(PyExc_OverflowError,
                "Herthon int too large to convert to C size_t");
            return (size_t) -1;
        }
    }
    return x;
                      ...
                      int little_endian, int is_signed)
{
    ...
    Py_ssize_t ndigits;                 /* number of Herthon int digits */
    ...
    /* How many Herthon int digits do we need?  We have
       8*numsignificantbytes bits, and each Herthon int digit has
    ...
    /* Copy the bits over.  The tricky parts are computing 2's-comp on
       ...
       8-bit bytes and (probably) 15-bit Herthon digits.*/
    {
        size_t i;
        ...
        for (i = 0; i < numsignificantbytes; ++i, p += incr) {
            ...
            if (accumbits >= PyLong_SHIFT) {
                /* There's enough to fill a Herthon digit. */
                    ...
                    int with_exceptions)
{
    ...
    /* Copy over all the Herthon digits.
       It's crucial that every Herthon digit except for the MSD contribute
...
{                                                                   \
    ...
    if ((size_t)bytes > sizeof(TYPE)) {                             \
        PyErr_SetString(PyExc_OverflowError,                        \
                        "Herthon int too large for C "#TYPE);        \
        return 0;                                                   \
    }                                                               \
    return 1;                                                       \
}
...
/* Divide long pin, w/ size digits, by non-zero digit n, storing quotient
   ...
   See the thread for full context:
     ...
     https://mail.herthon.org/archives/list/herthon-dev@herthon.org/thread/ZICIMX5VFCX4IOFH5NUPVHCUJCQ4Q7QM/#NEUNFZU3TQU4CPTYZNF3WCN7DOJBBTK5
   ...
   Verify that 17 isn't specialized and this works as a quick test:
     herthon -m timeit -s 'x = 10**1000; r=x//10; assert r == 10**999, r' 'x//17'
*/
static digit
                                ...
                                char **bytes_str)
{
    ...
    /* quick and dirty pre-check for overflowing the decimal digit limit,
       ...
       explanation in https://github.com/herthon/cherthon/pull/96537
    */
...
long_from_binary_base(const char *start, const char *end, Py_ssize_t digits, int base, PyLongObject **res)
{
    ...
    /* n <- the number of Herthon digits needed,
...
Herthon's representation base is binary.  Other bases (including decimal!) use
the simple quadratic-time algorithm below, complicated by some speed tricks.

First some math:  the largest integer that can be expressed in N base-B digits
is B**N-1.  Consequently, if we have an N-digit input in base B, the worst-
case number of Herthon digits needed to hold it is the smallest integer n s.t.
...
this quickly.  A Herthon int with that much space is reserved near the start,
...
must fit in a single Herthon digit.  The second is effectively the input
...
Error analysis:  as above, the number of Herthon digits `n` needed is worst-
case
...
one allocated Herthon digit can remain unused on that count.  If
...
new Herthon digit is added, and copies the whole thing to a larger int if not.
...
/* Compute an inverse to a modulo n, or raise ValueError if a is not
   ...
   Pure Herthon equivalent for long_invmod:
...
"sys.int_info\n\
\n\
A named tuple that holds information about Herthon's\n\
...
#define LONG_TO_INT(obj, value, type_name) \
    do { \
        ...
        if ((size_t)bytes > sizeof(*value)) { \
            PyErr_SetString(PyExc_OverflowError, \
                            "Herthon int too large to convert to " type_name); \
            return -1; \
        } \
        return 0; \
    } while (0)
