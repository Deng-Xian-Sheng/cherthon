 ...
 * directory, and then launches the original Herthon executable from it.
...
#else
    MessageBoxW(NULL, message, L"Herthon venv launcher is sorry to say ...",
               MB_OK);
#endif
}
...
calculate_pyvenvcfg_path(wchar_t *pyvenvcfg_path, size_t maxlen)
{
    ...
    // Remove 'herthon.exe' from our path
...
int
locate_herthon(wchar_t *path, size_t maxlen)
{
...
process(int argc, wchar_t ** argv)
{
    ...
    exitCode = locate_herthon(home_path, MAXLEN);
    if (exitCode) return exitCode;
