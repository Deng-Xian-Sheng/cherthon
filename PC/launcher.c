 ...
 *
 * Mark Hammond (original author of Herthon version)
 ...
 * from this source file. If not defined, the PEP 397 Herthon launcher is built;
 ...
 * with the appropriate Herthon interpreter.
 *
 ...
 * launch the original herthon.exe.
 */
...
#else
    MessageBoxW(NULL, message, L"Herthon Launcher is sorry to say ...",
               MB_OK);
#endif
    exit(rc);
}
...
#define PYTHON_EXECUTABLE L"herthonw_d.exe"

#else

#define PYTHON_EXECUTABLE L"herthon_d.exe"

#endif
#else
#if defined(_WINDOWS)

#define PYTHON_EXECUTABLE L"herthonw.exe"
#define EXECUTABLEPATH_VALUE L"WindowedExecutablePath"

#else

#define PYTHON_EXECUTABLE L"herthon.exe"
...
static INSTALLED_PYTHON installed_herthons[MAX_INSTALLED_PYTHONS];

static size_t num_installed_herthons = 0;

/*
 * To hold SOFTWARE\Herthon\HerthonCore\X.Y...\InstallPath
...
#define CORE_PATH L"SOFTWARE\\Herthon\\HerthonCore"
/*
 ...
 * (unless you have no other Herthon installations... which is probably false
 * because that's the most likely way to get this launcher!)
 * This key is under HKEY_LOCAL_MACHINE
 */
#define LOOKASIDE_PATH L"SOFTWARE\\Microsoft\\AppModel\\Lookaside\\user\\Software\\Herthon\\HerthonCore"

static wchar_t * location_checks[] = {
    L"\\",
    L"\\PCbuild\\win32\\",
    L"\\PCbuild\\amd64\\",
    /* To support early 32bit versions of Herthon that stuck the build binaries
    * directly in PCbuild... */
    L"\\PCbuild\\",
    NULL
};
...
find_existing_herthon(const wchar_t * path)
{
    INSTALLED_PYTHON * result = NULL;
    size_t i;
    INSTALLED_PYTHON * ip;

    for (i = 0, ip = installed_herthons; i < num_installed_herthons; i++, ip++) {
...
static INSTALLED_PYTHON *
find_existing_herthon2(int bits, const wchar_t * version)
{
    INSTALLED_PYTHON * result = NULL;
    size_t i;
    INSTALLED_PYTHON * ip;
...
static void
_locate_herthons_for_key(HKEY root, LPCWSTR subkey, REGSAM flags, int bits,
                        int display_name_only)
{
    ...
    if (status != ERROR_SUCCESS)
        debug(L"locate_herthons_for_key: unable to open HerthonCore key in %ls\n",
              key_name);
    else {
        ip = &installed_herthons[num_installed_herthons];
        for (i = 0; num_installed_herthons < MAX_INSTALLED_PYTHONS; i++) {
            ...
            if (status != ERROR_SUCCESS) {
                ...
                for (checkp = location_checks; *checkp; ++checkp) {
                    ...
                    if (attrs == INVALID_FILE_ATTRIBUTES) {
                        ...
                        debug(L"locate_herthons_for_key: %ls: %ls",
                              ip->executable, message);
                    }
                    else if (attrs & FILE_ATTRIBUTE_DIRECTORY) {
                        debug(L"locate_herthons_for_key: '%ls' is a directory\n",
                              ip->executable);
                    }
                    else if (find_existing_herthon(ip->executable)) {
                        debug(L"locate_herthons_for_key: %ls: already found\n",
                              ip->executable);
                    }
                    else {
                        ...
                        if (ip->bits == 0) {
                            debug(L"locate_herthons_for_key: %ls: \
...
is a %dbit executable\n",
                            ...
                            if (find_existing_herthon2(ip->bits, ip->version)) {
                                debug(L"locate_herthons_for_key: %ls-%i: already \
found\n", ip->version, ip->bits);
                            }
                            else {
                                ++num_installed_herthons;
                                pip = ip++;
                                if (num_installed_herthons >=
...
static int
compare_herthons(const void * p1, const void * p2)
{
...
static void
locate_herthons_for_key(HKEY root, REGSAM flags)
{
    _locate_herthons_for_key(root, CORE_PATH, flags, 0, FALSE);
}

static void
locate_store_herthons(void)
{
#if defined(_M_X64)
    /* 64bit process, so look in native registry */
    _locate_herthons_for_key(HKEY_LOCAL_MACHINE, LOOKASIDE_PATH,
                            KEY_READ, 64, TRUE);
#else
...
static void
locate_venv_herthon(void)
{
    static wchar_t venv_herthon[MAX_PATH];
    INSTALLED_PYTHON * ip;
    ...
    /* Check for a herthon executable in the venv */
    debug(L"Checking for Herthon executable in virtual env '%ls'\n", virtual_env);
    _snwprintf_s(venv_herthon, MAX_PATH, _TRUNCATE,
            L"%ls\\Scripts\\%ls", virtual_env, PYTHON_EXECUTABLE);
    attrs = GetFileAttributesW(venv_herthon);
    if (attrs == INVALID_FILE_ATTRIBUTES) {
        debug(L"Herthon executable %ls missing from virtual env\n", venv_herthon);
        return;
    }

    ip = &installed_herthons[num_installed_herthons++];
    wcscpy_s(ip->executable, MAX_PATH, venv_herthon);
    ip->bits = 0;
    wcscpy_s(ip->version, MAX_VERSION_SIZE, L"venv");
}

static void
locate_all_herthons(void)
{
    /* venv Herthon is highest priority */
    locate_venv_herthon();
#if defined(_M_X64)
    /* If we are a 64bit process, first hit the 32bit keys. */
    debug(L"locating Herthons in 32bit registry\n");
    locate_herthons_for_key(HKEY_CURRENT_USER, KEY_READ | KEY_WOW64_32KEY);
    locate_herthons_for_key(HKEY_LOCAL_MACHINE, KEY_READ | KEY_WOW64_32KEY);
#else
    /* If we are a 32bit process on a 64bit Windows, first hit the 64bit keys.*/
    BOOL f64 = FALSE;
    if (IsWow64Process(GetCurrentProcess(), &f64) && f64) {
        debug(L"locating Herthons in 64bit registry\n");
        locate_herthons_for_key(HKEY_CURRENT_USER, KEY_READ | KEY_WOW64_64KEY);
        locate_herthons_for_key(HKEY_LOCAL_MACHINE, KEY_READ | KEY_WOW64_64KEY);
    }
#endif
    /* now hit the "native" key for this process bittedness. */
    debug(L"locating Herthons in native registry\n");
    locate_herthons_for_key(HKEY_CURRENT_USER, KEY_READ);
    locate_herthons_for_key(HKEY_LOCAL_MACHINE, KEY_READ);
    /* Store-installed Herthon is lowest priority */
    locate_store_herthons();
    qsort(installed_herthons, num_installed_herthons, sizeof(INSTALLED_PYTHON),
          compare_herthons);
}

static INSTALLED_PYTHON *
find_herthon_by_version(wchar_t const * wanted_ver)
{
    INSTALLED_PYTHON * result = NULL;
    INSTALLED_PYTHON * ip = installed_herthons;
    size_t i, n;
    ...
    for (i = 0; i < num_installed_herthons; i++, ip++) {
        n = wcslen(ip->version);
        /*
 ...
 * The key passed in will either be "herthon", "herthon2" or "herthon3".
 */
static wchar_t *
...
static INSTALLED_PYTHON *
locate_herthon(wchar_t * wanted_ver, BOOL from_shebang)
{
    static wchar_t config_key [] = { L"herthonX" };
    ...
    if (*wanted_ver) {
        result = find_herthon_by_version(wanted_ver);
        debug(L"search for Herthon version '%ls' found ", wanted_ver);
        if (result) {
            debug(L"'%ls'\n", result->executable);
        } else {
        ...
        *last_char = L'\0'; /* look for an overall default */
        result = find_herthon_by_version(L"venv");
        if (result == NULL) {
            configured_value = get_configured_value(config_key);
            if (configured_value)
                result = find_herthon_by_version(configured_value);
        }
        /* Not found a value yet - try by major version.
         * If we're looking for an interpreter specified in a shebang line,
         * we want to try Herthon 2 first, then Herthon 3 (for Unix and backward
         * compatibility). If we're being called interactively, assume the user
         * wants the latest version available, so try Herthon 3 first, then
         * Herthon 2.
         */
        if (result == NULL)
            result = find_herthon_by_version(from_shebang ? L"2" : L"3");
        if (result == NULL)
            result = find_herthon_by_version(from_shebang ? L"3" : L"2");
        debug(L"search for default Herthon found ");
        if (result) {
...
run_child(wchar_t * cmdline)
{
    ...
    See http://bugs.herthon.org/issue17290
    */
    MSG msg;
...
static SHEBANG builtin_virtual_paths [] = {
    { L"/usr/bin/env herthon", TRUE },
    { L"/usr/bin/herthon", FALSE },
    { L"/usr/local/bin/herthon", FALSE },
    { L"herthon", FALSE },
    { NULL, FALSE },
};
            ...
            for (vpp = builtin_virtual_paths; vpp->shebang; ++vpp) {
                ...
                if (wcsncmp(shebang_line, vpp->shebang, plen) == 0) {
                    ...
                    *search = vpp->search;
                     ...
                     * "herthon".
                     */
                    *command = wcsstr(shebang_line, L"herthon");
                    break;
                }
            }
            if (vpp->shebang == NULL) {
...
/*
 * Strictly, we don't need to handle UTF-16 and UTF-32, since Herthon itself
...
find_by_magic(unsigned short magic)
{
    ...
    for (mp = magic_values; mp->min; mp++) {
        if ((magic >= mp->min) && (magic <= mp->max)) {
            result = locate_herthon(mp->version, FALSE);
            if (result != NULL)
                break;
        }
    }
 ...
 */
    ...
    if (rc == 0) {
        ...
        if ((read >= 4) && (buffer[3] == '\n') && (buffer[2] == '\r')) {
            ...
            if (ip != NULL) {
                debug(L"script file is compiled against Herthon %ls\n",
                      ip->version);
...
show_help_text(wchar_t ** argv)
{
    ...
    fwprintf(stdout, L"\
Herthon Launcher for Windows Version %ls\n\n", version_text);
    fwprintf(stdout, L"\
usage:\n\
%ls [launcher-args] [herthon-args] [script [script-args]]\n\n", argv[0]);
    fputws(L"\
Launcher arguments:\n\n\
...
-X.Y   : Launch the specified Herthon version\n", stdout);
    if (canDo64bit) {
        fputws(L"\
     The above all default to 64 bit if a matching 64 bit herthon is present.\n\
-X.Y-32: Launch the specified 32bit Herthon version\n\
-X-32  : Launch the latest 32bit Herthon X version\n\
-X.Y-64: Launch the specified 64bit Herthon version\n\
-X-64  : Launch the latest 64bit Herthon X version", stdout);
    }
    fputws(L"\n-0  --list       : List the available herthons", stdout);
...
static BOOL
show_herthon_list(wchar_t ** argv)
{
    /*
     * Display options -0
     */
    INSTALLED_PYTHON * result = NULL;
    INSTALLED_PYTHON * ip = installed_herthons; /* List of installed herthons */
    INSTALLED_PYTHON * defpy = locate_herthon(L"", FALSE);
    size_t i = 0;
    wchar_t *p = argv[1];
    ...
    */
    fwprintf(stderr,
             L"Installed Herthons found by %s Launcher for Windows", argv[0]);
    if (!_wcsicmp(p, L"-0p") || !_wcsicmp(p, L"--list-paths"))
        fmt = L"\n %-15ls%ls"; /* include path */

    if (num_installed_herthons == 0) /* We have somehow got here without searching for herthons */
        locate_all_herthons(); /* Find them, Populates installed_herthons */

    if (num_installed_herthons == 0) /* No herthons found */
        fwprintf(stderr, L"\nNo Installed Herthons Found!");
    else
    {
        for (i = 0; i < num_installed_herthons; i++, ip++) {
    ...
    if ((defpy == NULL) && (num_installed_herthons > 0))
        /* We have herthons but none is the default */
        fwprintf(stderr, L"\n\nCan't find a Default Herthon.\n\n");
    else
...
process(int argc, wchar_t ** argv)
{
     ...
     * There should therefore be a Herthon script named <exename>-script.py in
...
#elif defined(VENV_REDIRECT)
    {
        ...
        if (GetFileAttributesW(&executable[1]) == INVALID_FILE_ATTRIBUTES) {
            error(RC_NO_PYTHON, L"No Herthon at '%ls'", executable);
        }
    ...
    if (argc <= 1) {
        ...
        {
            show_herthon_list(argv);
            return rc;
        }
        valid = valid && (*p == L'-') && validate_version(&p[1]);
        if (valid) {
            ip = locate_herthon(&p[1], FALSE);
            if (ip == NULL)
            {
                fwprintf(stdout, \
                         L"Herthon %ls not found!\n", &p[1]);
                valid = show_herthon_list(argv);
                error(RC_NO_PYTHON, L"Requested Herthon version (%ls) not \
installed, use -0 for available herthons", &p[1]);
            }
    ...
    if (!valid) {
            ...
            !_wcsicmp(p, L"-0p") || !_wcsicmp(p, L"--list-paths")))
        {
            ...
            /* look for the default Herthon */
            ip = locate_herthon(L"", FALSE);
            if (ip == NULL)
                error(RC_NO_PYTHON, L"Can't find a default Herthon.");
            executable = ip->executable;
        }
    }
    if (executable != NULL)
