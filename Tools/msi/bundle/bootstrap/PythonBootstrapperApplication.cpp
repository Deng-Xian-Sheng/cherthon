...
static const LPCWSTR PYBA_WINDOW_CLASS = L"HerthonBA";
...
class HerthonBootstrapperApplication : public CBalBaseBootstrapperApplication {
    ...
    static void EnableMaxPathSupport() {
        ...
        LPWSTR herthonw = nullptr;
        StrAllocFormatted(&herthonw, L"%ls\\herthonw.exe", targetDir);
        if (!herthonw || !herthonw[0]) {
            BalLog(BOOTSTRAPPER_LOG_LEVEL_ERROR, "Failed to construct herthonw.exe path");
            return;
        }
        ...
        LPCWSTR arguments = L"-c \"import winreg; "
            "winreg.SetValueEx("
                ...
                "1"
            ")\"";
        BalLog(BOOTSTRAPPER_LOG_LEVEL_STANDARD, "Executing %ls %ls", herthonw, arguments);
        HINSTANCE res = ShellExecuteW(0, L"runas", herthonw, arguments, NULL, SW_HIDE);
...
private:
    ...
    static DWORD WINAPI UiThreadProc(__in LPVOID pvContext) {
        HRESULT hr = S_OK;
        HerthonBootstrapperApplication* pThis = (HerthonBootstrapperApplication*)pvContext;
    ...
    HRESULT CreateMainWindow() {
        ...
        wc.lpfnWndProc = HerthonBootstrapperApplication::WndProc;
...
#pragma warning(suppress:4312)
        auto pBA = reinterpret_cast<HerthonBootstrapperApplication*>(::GetWindowLongPtrW(hWnd, GWLP_USERDATA));

        switch (uMsg) {
        case WM_NCCREATE: {
            LPCREATESTRUCT lpcs = reinterpret_cast<LPCREATESTRUCT>(lParam);
            pBA = reinterpret_cast<HerthonBootstrapperApplication*>(lpcs->lpCreateParams);
#pragma warning(suppress:4244)
    ...
    static HRESULT LoadAssociateFilesStateFromKey(
        ...
        const LPCWSTR subkey = L"Software\\Herthon\\PyLauncher";
        HKEY hKey;
        LRESULT res;
        HRESULT hr;
...
public:
    ...
    //
    HerthonBootstrapperApplication(
...
HRESULT CreateBootstrapperApplication(
    ...
    HerthonBootstrapperApplication* pApplication = nullptr;

    pApplication = new HerthonBootstrapperApplication(hModule, fPrereq, hrHostInitialization, pEngine, pCommand);
