<#
.Synopsis
    Uploads from a VSTS release build layout to herthon.org
.Description
    Given the downloaded/extracted build artifact from a release
    build run on herthon.visualstudio.com, this script uploads
...
param(
    ...
    [string]$server="herthon-downloads",
    [string]$target="/srv/www.herthon.org/ftp/herthon",
...
if (-not ((Test-Path "$build\win32\herthon-*.exe") -or (Test-Path "$build\amd64\herthon-*.exe"))) {
...
$p = gci -r "$build\herthon-*.exe" | `
    ?{ $_.Name -match '^herthon-(\d+\.\d+\.\d+)((a|b|rc)\d+)?-.+' } | `
    select -first 1 | `
    %{ $Matches[1], $Matches[2] }
...
if (-not $skipupload) {
    ...
    if ($doc_htmlhelp) {
        $chm = gci -EA 0 $doc_htmlhelp\herthon*.chm, $doc_htmlhelp\herthon*.chm.asc
    } else {
        $chm = gci -EA 0 $build\herthon*.chm, $build\herthon*.chm.asc
    }
...
if (-not $skiphash) {
    ...
    pushd $build
    $files = gci herthon*.chm, *\*.exe, *\*.zip
    if ($doc_htmlhelp) {
        cd $doc_htmlhelp
        $files = ($files, (gci herthon*.chm)) | %{ $_ }
    }
    if ($embed) {
        cd $embed
