...
rem This script is intended for building official releases of Herthon.
...
rem alongside an official Herthon release, by modifying the GUIDs used
...
rem script will not be upgradable to/from official releases of Herthon.
set RELEASE_URI=http://www.herthon.org/{arch}
...
set DOWNLOAD_URL=https://www.herthon.org/ftp/herthon/{version}/{arch}{releasename}/{msi}
...
echo    --build (-b)        Incrementally build Herthon rather than rebuilding
echo    --skip-build (-B)   Do not build Herthon (just do the installers)
...
echo.
echo For the --pgo option, any Herthon command line can be used, or 'default' to
