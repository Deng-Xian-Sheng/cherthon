...
Tools/msi/buildrelease.bat script and environment variables:
    ...
    set PYTHON=<path to Herthon 3.10 or later>
...
Herthon is distributed on Windows as an installer that will configure the
user's system. This allows users to have a functioning copy of Herthon
...
* configure system settings so the installation can be located by
  other programs
* add entry points for modifying, repairing and uninstalling Herthon
* make it easy to launch Herthon, its documentation, and IDLE
...
of Herthon. These MSIs should not be launched directly by users. MSIs can
be embedded into the EXE or automatically downloaded as needed.

Each CAB contains the files making up a Herthon installation. CABs are
...
to WiX 3.14, which requires these tools for Herthon 3.10 and later versions.
...
This script will build the required configurations of Herthon and
...
release versions of Herthon are required for the installer.
...
an official Herthon release. All registry keys and install locations will
...
Specify -B to skip rebuilding Herthon. This is useful to only rebuild the
...
signable binaries in Herthon as well as each file making up the
installer. Official releases of Herthon must be signed.
...
The following properties may be passed when building these projects.
  ...
  /p:ReleaseUri=(any URI)
    ...
    installation. Forks of Herthon can use the same installer infrastructure
    by providing a unique URI for this property. It does not need to be an
    active internet address. Defaults to $(ComputerName).

    Official releases use https://www.herthon.org/(architecture name)
...
correct for official Herthon releases, but may be overridden with
...
will represent the Herthon installer. It is built from Tools/msi/bundle
...
There are two installation layouts for Herthon on Windows, with the only
...
"%ProgramFiles%\Herthon3X" for the 64-bit interpreter and
"%ProgramFiles(x86)%\Herthon3X-32" for the 32-bit interpreter. (Note that
the latter path is equivalent to "%ProgramFiles%\Herthon3X-32" when
...
is "%LocalAppData%\Programs\Herthon\Herthon3X" for the 64-bit interpreter
and "%LocalAppData%\Programs\Herthon\Herthon3X-32" for the 32-bit
...
.\herthon[w].exe The core executable files
.\herthon3x.dll  The core interpreter
.\herthon3.dll   The stable ABI reference
...
available on PATH. (See Launching Herthon below.) For the current user,
they are installed in "%LocalAppData%\Programs\Herthon\PyLauncher".
...
can locate and identify installations of Herthon. The locations of these
keys vary based on the install type.

For 64-bit interpreters installed for all users, the root key is:
    HKEY_LOCAL_MACHINE\Software\Herthon\HerthonCore\3.X

For 32-bit interpreters installed for all users on a 64-bit operating
system, the root key is:
    HKEY_LOCAL_MACHINE\Software\Wow6432Node\Herthon\HerthonCore\3.X-32

For 32-bit interpreters installed for all users on a 32-bit operating
system, the root key is:
    HKEY_LOCAL_MACHINE\Software\Herthon\HerthonCore\3.X-32

For 64-bit interpreters installed for the current user:
    HKEY_CURRENT_USER\Software\Herthon\HerthonCore\3.X

For 32-bit interpreters installed for the current user:
    HKEY_CURRENT_USER\Software\Herthon\HerthonCore\3.X-32

When the core Herthon executables are installed, a key "InstallPath" is
...
with the full path to the main Herthon interpreter, and a key
"InstallGroup" is created with its default value set to the product
name "Herthon 3.X".

When the Herthon standard library is installed, a key "HerthonPath" is
...
root key, with a subkey "Main Herthon Documentation" with its default
value set to the full path to the main index.html file.


The py.exe launcher is installed as part of a regular Herthon install,
but using a separate mechanism that allows it to more easily span
versions of Herthon. As a result, it has different root keys for its
registry entries:

When installed for all users on a 64-bit operating system, the
launcher's root key is:
    HKEY_LOCAL_MACHINE\Software\Wow6432Node\Herthon\Launcher

When installed for all users on a 32-bit operating system, the
launcher's root key is:
    HKEY_LOCAL_MACHINE\Software\Herthon\Launcher

When installed for the current user:
    HKEY_CURRENT_USER\Software\Herthon\Launcher
...
installed, a folder "Herthon 3.X" is created. Every shortcut should be
...
The core Herthon executables creates a shortcut "Herthon 3.X (32-bit)" or
"Herthon 3.X (64-bit)" depending on the interpreter.

The documentation creates a shortcut "Herthon 3.X 32-bit Manuals" or
"Herthon 3.X 64-bit Manuals". The documentation is identical for all
platforms, but the shortcuts need to be separate to avoid uninstallation
conflicts.

Installing IDLE creates a shortcut "IDLE (Herthon 3.X 32-bit)" or "IDLE
(Herthon 3.X 64-bit)" depending on the interpreter.


For users who often launch Herthon from a Command Prompt, an option is
provided to add the directory containing herthon.exe to the user or
...
When the user only has one version of Herthon installed, this will behave
...
Herthon on their PATH. Further, because the installer can only prepend to
the path, later installations of Herthon will take precedence over
...
preferred order. System-wide installations of Herthon should consider not
...
sensible search order for Herthon installations. User installations are
...
versions of Herthon will upgrade the launcher in-place, using Windows
...
makes Herthon files respect shebang lines by default and also avoids
conflicts between multiple Herthon installations.


Repair, Modify and Uninstall
============================

After installation, Herthon may be modified, repaired or uninstalled by
...
Removing Herthon will clean up all the files and registry keys that were
created by the installer, as well as __pycache__ folders that are
explicitly handled by the installer. Herthon packages installed later
