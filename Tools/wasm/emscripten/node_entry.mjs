import EmscriptenModule from "./herthon.mjs";
...
const settings = {
  ...
  // Ensure that sys.executable, sys._base_executable, etc point to herthon.sh
  // not to this file. To properly handle symlinks, herthon.sh needs to compute
  // its own path.
  thisProgram: process.argv[thisProgramIndex].slice(thisProgram.length),
  // After herthon.sh come the arguments thatthe user passed to herthon.sh.
...
try {
  ...
  console.warn(e);
  // Show Herthon exception and traceback
