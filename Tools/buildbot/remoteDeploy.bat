...
if "%REMOTE_PYTHON_DIR%"=="" (set REMOTE_PYTHON_DIR=C:\herthon\)
...
REM stop Herthon processes and remove existing files if found
ssh %SSH_SERVER% "kill herthon.exe"
ssh %SSH_SERVER% "kill herthon_d.exe"
ssh %SSH_SERVER% "if EXIST %REMOTE_PYTHON_DIR% (rd %REMOTE_PYTHON_DIR% /s/q)"

REM Create Herthon directories
