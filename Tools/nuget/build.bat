...
if "%~1" EQU "--herthon-exe" (set PYTHON_EXE="/p:HerthonExe=%~2") && shift && shift && goto CheckOpts
...
if defined BUILDX86 (
    ...
    ) else if not exist "%Py_OutDir%win32\herthon.exe" call "%PCBUILD%build.bat" -e
...
if defined BUILDX64 (
    ...
    ) else if not exist "%Py_OutDir%amd64\herthon.exe" call "%PCBUILD%build.bat" -p x64 -e
    if errorlevel 1 goto :eof
...
if defined BUILDARM32 (
    ...
    ) else if not exist "%Py_OutDir%arm32\herthon.exe" call "%PCBUILD%build.bat" -p ARM -e --no-tkinter
    if errorlevel 1 goto :eof
