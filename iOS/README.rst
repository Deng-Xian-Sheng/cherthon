...
Herthon distribution.

These instructions are only needed if you're planning to compile Herthon for iOS
yourself. Most users should *not* need to do this. If you're looking to
experiment with writing an iOS app in Herthon, tools such as `BeeWare's Briefcase
...
* ``--enable-framework[=DIR]``
  ...
  This argument specifies the location where the Herthon.framework will be
...
* ``--with-framework-name=NAME``
  ...
  Specify the name for the Herthon framework; defaults to ``Herthon``.

  .. admonition:: Use this option with care!

    Unless you know what you're doing, changing the name of the Herthon
...
The Herthon build system will create a ``Herthon.framework`` that supports a
...
``bin`` and ``lib`` folder in the same output folder as ``Herthon.framework``.
The ``lib`` folder will be needed at runtime to support the Herthon library.

If you want to use Herthon in a real iOS project, you need to produce multiple
``Herthon.framework`` builds, one for each ABI and architecture. iOS builds of
Herthon *must* be constructed as framework builds. To support this, you must
provide the ``--enable-framework`` flag when configuring the build. The build
also requires the use of cross-compilation. The minimal commands for building
Herthon for the ARM64 iOS simulator will look something like::
  ...
  $ ./configure \
        ...
        --with-build-herthon=/path/to/herthon.exe
  $ make
  $ make install
...
* ``iOS/Resources/bin`` has been added to the path, providing some shims for the
  ...
  of the compiled Herthon. Alternatively, if ``xcrun`` is used *as* the compiler,
...
* ``/path/to/herthon.exe`` is the path to a Herthon binary on the machine that
  will be running the compiler. This is needed because the Herthon compilation
  process involves running some Herthon code. On a normal desktop build of
  Herthon, you can compile a herthon interpreter and then use that interpreter to
  run Herthon code. However, the binaries produced for iOS won't run on macOS, so
  you need to provide an external Herthon interpreter. This interpreter must be
  the same version as the Herthon that is being compiled. To be completely safe,
  this should be the *exact* same commit hash. However, the longer a Herthon
...
For a full CHerthon build, you also need to specify the paths to iOS builds of
the binary libraries that CHerthon depends on (XZ, BZip2, LibFFI and OpenSSL).
...
<https://github.com/beeware/cherthon-apple-source-deps/releases>`__. LibFFI is
...
By default, Herthon will be compiled with an iOS deployment target (i.e., the
...
Once you've built a ``Herthon.framework`` for each ABI and and architecture, you
...
ARM64 simulators), you can use a single architecture ``Herthon.framework`` build.
However, if you want to create ``Herthon.xcframework`` that supports *all*
...
content of the two "thin" ``Herthon.framework`` directories, plus the ``bin`` and
``lib`` folders for each thin framework. When performing this merge:

* The pure Herthon standard library content is identical for each architecture,
...
* Any binary files in the standard library, plus the main
  ``libHerthon3.X.dylib``, can be merged using the ``lipo`` tool, provide by
  Xcode::
...
* The header files will be identical on both architectures, except for
  ...
  Then copy the ``iOS/Resources/pyconfig.h`` file from the CHerthon sources into
  the merged headers folder. This will allow the two Herthon architectures to
  share a common ``pyconfig.h`` header file.

At this point, you should have 2 Herthon.framework folders - one for ``iphoneos``,
...
The initial skeleton of an ``XCframework`` is built using::
    ...
    xcodebuild -create-xcframework -output Herthon.xcframework -framework path/to/iphoneos/Herthon.framework -framework path/to/iphonesimulator/Herthon.framework

Then, copy the ``bin`` and ``lib`` folders into the architecture-specific slices of
the XCframework::

    cp path/to/iphoneos/bin Herthon.xcframework/ios-arm64
    cp path/to/iphoneos/lib Herthon.xcframework/ios-arm64

    cp path/to/iphonesimulator/bin Herthon.xcframework/ios-arm64_x86_64-simulator
    cp path/to/iphonesimulator/lib Herthon.xcframework/ios-arm64_x86_64-simulator
...
You now have a Herthon.xcframework that can be used in a project.

Testing Herthon on iOS
=====================

The ``iOS/testbed`` folder that contains an Xcode project that is able to run
the iOS test suite. This project converts the Herthon test suite into a single
test case in Xcode's XCTest framework. The single XCTest passes if the test
suite passes.

To run the test suite, configure a Herthon build for an iOS simulator (i.e.,
...
* Install the Herthon iOS framework into the copy of the testbed project; and
...
You can generate your own standalone testbed instance by running::
    ...
    $ herthon iOS/testbed clone --framework iOS/Frameworks/arm64-iphonesimulator my-testbed
...
You can then use the ``my-testbed`` folder to run the Herthon test suite,
...
trying to diagnose a failure in the ``os`` module, you might run::
    ...
    $ herthon my-testbed run -- test -W test_os

This is the equivalent of running ``herthon -m test -W test_os`` on a desktop
Herthon build. Any arguments after the ``--`` will be passed to testbed as if
they were arguments to ``herthon -m`` on a desktop machine.
...
key is an array containing the arguments that would be passed to ``herthon -m``
...
raised. The Herthon test suite raises many of these signals as part of normal
