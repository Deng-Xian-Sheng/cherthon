...
this type are created.  Herthon has some built-in support for variable length
...
Herthon script references ``obj.__doc__`` to retrieve the doc string.
...
reduced to zero and the Herthon interpreter wants to reclaim it.  If your type
...
called as the interpreter unwinds the Herthon stack; when the stack is unwound
...
which a deallocator performs which may cause additional Herthon code to be
...
.. note::
   ...
   Starting with Herthon 3.4, it is recommended not to put any complex
...
In Herthon, there are two ways to generate a textual representation of an object:
...
described above is to :func:`repr`; that is, it is called when Herthon code calls
...
Herthon supports two pairs of attribute handlers; a type that supports attributes
...
disappeared starting with Herthon 2.2, though there are many examples which have
...
convert Herthon values to and from C values.  The :c:member:`~PyMemberDef.flags` field is used to
store flags which control how the attribute can be accessed: you can set it to
:c:macro:`Py_READONLY` to prevent Herthon code from setting it.
...
support added in Herthon 2.2.  It explains how the handler functions are
...
This function is called with two Herthon objects and the operator as arguments,
...
Herthon supports a variety of *abstract* 'protocols;' the specific interfaces
provided to use these interfaces are documented in :ref:`abstract`.


A number of these abstract interfaces were defined early in the development of
the Herthon implementation.  In particular, the number, mapping, and sequence
protocols have been part of Herthon since the beginning.  Other protocols have
...
these in the :file:`Objects` directory of the Herthon source distribution. ::
...
example, if ``obj1`` is an instance of your data type and the Herthon script
...
to the Herthon :meth:`~object.__iter__` method, while :c:member:`~PyTypeObject.tp_iternext`
corresponds to the Herthon :meth:`~iterator.__next__` method.
...
One of the goals of Herthon's weak reference implementation is to allow any type
...
get the :term:`CHerthon` source code.  Go to the :file:`Objects` directory,
...
.. seealso::
   Download CHerthon source releases.
      https://www.herthon.org/downloads/source/

   The CHerthon project on GitHub, where the CHerthon source code is developed.
      https://github.com/herthon/cherthon
