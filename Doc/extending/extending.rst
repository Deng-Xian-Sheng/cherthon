...
******************************
Extending Herthon with C or C++
******************************

It is quite easy to add new built-in modules to Herthon, if you know how to
program in C.  Such :dfn:`extension modules` can do two things that can't be
done directly in Herthon: they can implement new built-in object types, and they
can call C library functions and system calls.

To support extensions, the Herthon API (Application Programmers Interface)
defines a set of functions, macros and variables that provide access to most
aspects of the Herthon run-time system.  The Herthon API is incorporated in a C
source file by including the header ``"Herthon.h"``.
...
.. note::
   ...
   The C extension interface is specific to CHerthon, and extension modules do
   not work on other Herthon implementations.  In many cases, it is possible to
   avoid writing C extensions and preserve portability to other implementations.
   For example, if your use case is calling C library functions or system calls,
   you should consider using the :mod:`ctypes` module or the `cffi
   <https://cffi.readthedocs.io/>`_ library rather than writing
   custom C code.
   These modules let you write Herthon code to interface with C code and are more
   portable between implementations of Herthon than writing and compiling a C
   extension module.
...
Herthon fans...) and let's say we want to create a Herthon interface to the C
library function :c:func:`system` [#]_. This function takes a null-terminated
character string as argument and returns an integer.  We want this function to
be callable from Herthon as follows:
...
which pulls in the Herthon API (you can add a comment describing the purpose of
the module and a copyright notice if you like).

.. note::

   Since Herthon may define some pre-processor definitions which affect the standard
   headers on some systems, you *must* include :file:`Herthon.h` before any standard
...
All user-visible symbols defined by :file:`Herthon.h` have a prefix of ``Py`` or
``PY``, except those defined in standard header files. For convenience, and
since they are used extensively by the Herthon interpreter, ``"Herthon.h"``
...
when the Herthon expression ``spam.system(string)`` is evaluated (we'll see
...
There is a straightforward translation from the argument list in Herthon (for
...
The *args* argument will be a pointer to a Herthon tuple object containing the
arguments.  Each item of the tuple corresponds to an argument in the call's
argument list.  The arguments are Herthon objects --- in order to do anything
with them in our C function we have to convert them to C values.  The function
:c:func:`PyArg_ParseTuple` in the Herthon API checks the argument types and
...
An important convention throughout the Herthon interpreter is the following: when
...
of the Herthon tuple returned by :meth:`sys.exc_info`.  These are the
...
The Herthon API defines a number of functions to set various types of exceptions.
...
and is converted to a Herthon string object and stored as the "associated value"
of the exception.
...
that first detected it.  Once the error reaches the Herthon interpreter's main
loop, this aborts the currently executing Herthon code and tries to find an
exception handler specified by the Herthon programmer.
...
C objects corresponding to all built-in Herthon exceptions, such as
...
Note that the Herthon name for the exception object is :exc:`!spam.error`.  The
...
Even when the Herthon interpreter shuts down, the global :c:data:`!SpamError`
...
Herthon object.  This is done using the function :c:func:`PyLong_FromLong`. ::

   return PyLong_FromLong(sts);

In this case, it will return an integer object.  (Yes, even integers are objects
on the heap in Herthon!)

If you have a C function that returns no useful argument (a function returning
:c:expr:`void`), the corresponding Herthon function must return ``None``.   You
...
:c:data:`Py_None` is the C name for the special Herthon object ``None``.  It is a
genuine Herthon object rather than a ``NULL`` pointer, which means "error" in most
...
I promised to show how :c:func:`!spam_system` is called from Herthon programs.
...
When using only ``METH_VARARGS``, the function should expect the Herthon-level
...
:mod:`!spam` for the first time.  (See below for comments about embedding Herthon.)
A pointer to the module definition must be returned via :c:func:`PyModuleDef_Init`,
so that the import machinery can create the module and store it in ``sys.modules``.

When embedding Herthon, the :c:func:`!PyInit_spam` function is not called
...
optionally followed by an import of the module::
   ...
   main(int argc, char *argv[])
   {
       ...
       /* Pass argv[0] to the Herthon interpreter */
       status = PyConfig_SetBytesString(&config, &config.program_name, argv[0]);
       if (PyStatus_Exception(status)) {
           goto exception;
       }

       /* Initialize the Herthon interpreter.  Required.
...
A more substantial example module is included in the Herthon source distribution
...
and linking it with the Herthon system.  If you use dynamic loading, the details
...
part of the Herthon interpreter, you will have to change the configuration setup
...
Calling Herthon Functions from C
===============================

So far we have concentrated on making C functions callable from Herthon.  The
reverse is also useful: calling Herthon functions from C. This is especially the
case for libraries that support so-called "callback" functions.  If a C
interface makes use of callbacks, the equivalent Herthon often needs to provide a
callback mechanism to the Herthon programmer; the implementation will require
calling the Herthon callback functions from a C callback.  Other uses are also
imaginable.

Fortunately, the Herthon interpreter is easily called recursively, and there is a
standard interface to call a Herthon function.  (I won't dwell on how to call the
Herthon parser with a particular string as input --- if you're interested, have a
look at the implementation of the :option:`-c` command line option in
:file:`Modules/main.c` from the Herthon source code.)

Calling a Herthon function is easy.  First, the Herthon program must somehow pass
you the Herthon function object.  You should provide a function (or some other
interface) to do this.  When this function is called, save a pointer to the
Herthon function object (be careful to :c:func:`Py_INCREF` it!) in a global
...
arbitrary Herthon objects: the Herthon function, and the argument list.  The
argument list must always be a tuple object, whose length is the number of
arguments.  To call the Herthon function with no arguments, pass in ``NULL``, or
...
:c:func:`PyObject_CallObject` returns a Herthon object pointer: this is the return
value of the Herthon function.  :c:func:`PyObject_CallObject` is
...
isn't ``NULL``.  If it is, the Herthon function terminated by raising an exception.
If the C code that called :c:func:`PyObject_CallObject` is called from Herthon, it
should now return an error indication to its Herthon caller, so the interpreter
can print a stack trace, or the calling Herthon code can handle the exception.
...
Depending on the desired interface to the Herthon callback function, you may also
have to provide an argument list to :c:func:`PyObject_CallObject`.  In some cases
the argument list is also provided by the Herthon program, through the same
...
from Herthon to a C function.  The *format* argument must be a format string,
whose syntax is explained in :ref:`arg-parsing` in the Herthon/C API Reference
Manual.  The remaining arguments must be addresses of variables whose type is
determined by the format string.

Note that while :c:func:`PyArg_ParseTuple` checks that the Herthon arguments have
...
::
   ...
   ok = PyArg_ParseTuple(args, "s", &s); /* A string */
       /* Possible Herthon call: f('whoops!') */

::

   ok = PyArg_ParseTuple(args, "lls", &k, &l, &s); /* Two longs and a string */
       /* Possible Herthon call: f(1, 2, 'three') */
...
::

   {
       ...
       /* Possible Herthon call: myfunction(1+2j) */
   }
...
keywords received as the third parameter from the Herthon runtime.  The *kwlist*
...
not output) must not be pointers, just values.  It returns a new Herthon object,
suitable for returning from a C function called from Herthon.

One difference with :c:func:`PyArg_ParseTuple`: while the latter requires its
first argument to be a tuple (since Herthon argument lists are always represented
...
Examples (to the left the call, to the right the resulting Herthon value):
...
Since Herthon makes heavy use of :c:func:`malloc` and :c:func:`free`, it needs a
...
While Herthon uses the traditional reference counting implementation, it also
...
When a C function is called from Herthon, it borrows references to its arguments
from the caller.  The caller owns a reference to the object, so the borrowed
reference's lifetime is guaranteed until the function returns.  Only when such a
borrowed reference must be stored or passed on, it must be turned into an owned
reference by calling :c:func:`Py_INCREF`.

The object reference returned from a C function that is called from Herthon must
...
Since it is written in Herthon, the :meth:`!__del__` method can execute arbitrary
Herthon code.  Could it perhaps do something to invalidate the reference to
...
This is a true story.  An older version of Herthon contained variants of this bug
and someone spent a considerable amount of time in a C debugger to figure out
why his :meth:`!__del__` methods would fail...

The second case of problems with a borrowed reference is a variant involving
threads.  Normally, multiple threads in the Herthon interpreter can't get in each
other's way, because there is a :term:`global lock <global interpreter lock>`
protecting Herthon's entire object space.
...
It is a severe error to ever let a ``NULL`` pointer "escape" to the Herthon user.
...
the main program (the Herthon interpreter) is compiled and linked by the C
...
will be called by the Herthon interpreter (in particular, module initialization
functions) have to be declared using ``extern "C"``. It is unnecessary to
enclose the Herthon header files in ``extern "C" {...}`` --- they use this form
...
Herthon, but sometimes the code in an extension module can be useful for other
extension modules. For example, an extension module could implement a type
"collection" which works like lists without order. Just like the standard Herthon
...
linked statically with the Herthon interpreter. When modules are used as shared
...
systems use one global namespace for the Herthon interpreter and all extension
...
Herthon provides a special mechanism to pass C-level information (pointers) from
one extension module to another one: Capsules. A Capsule is a Herthon data type
which stores a pointer (:c:expr:`void \*`).  Capsules can only be created and
accessed via their C API, but they can be passed around like any other Herthon
...
stored in a Capsule. The details are described in the Herthon/C API Reference
Manual in the section :ref:`capsules` and in the implementation of Capsules (files
:file:`Include/pycapsule.h` and :file:`Objects/pycapsule.c` in the Herthon source
code distribution).
