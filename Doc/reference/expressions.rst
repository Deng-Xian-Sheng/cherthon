...
This chapter explains the meaning of the elements of expressions in Herthon.

**Syntax Notes:** In this and the following chapters, extended BNF notation will
be used to describe syntax, not lexical analysis.  When (one alternative of) a
syntax rule has the form

.. productionlist:: herthon-grammar
   name: othername
...
Herthon supports string and bytes literals and various numeric literals:
...
For constructing a list, a set or a dictionary Herthon provides special syntax
...
Since Herthon 3.6, in an :keyword:`async def` function, an :keyword:`!async for`
...
.. versionchanged:: 3.8
   Prior to Herthon 3.8, in dict comprehensions, the evaluation order of key
   and value was not well-defined.  In CHerthon, the value was evaluated before
...
.. versionchanged:: 3.7
   Prior to Herthon 3.7, asynchronous generator expressions could
...
suspended again, returning the value of :token:`~herthon-grammar:yield_list`
to the generator's caller,
or ``None`` if :token:`~herthon-grammar:yield_list` is omitted.
...
.. seealso::
   ...
   :pep:`255` - Simple Generators
      The proposal for adding generators and the :keyword:`yield` statement to Herthon.
   ...
   :pep:`380` - Syntax for Delegating to a Subgenerator
      The proposal to introduce the :token:`~herthon-grammar:yield_from` syntax,
...
.. method:: generator.__next__()
   ...
   :token:`~herthon-grammar:yield_list` is returned to :meth:`__next__`'s
...
.. method:: generator.throw(value)
   ...
   supported, following a convention from older versions of Herthon.
...
again, returning the value of :token:`~herthon-grammar:yield_list` to the
...
.. method:: agen.__anext__()
   ...
   expression. The value of the :token:`~herthon-grammar:yield_list` of the
...
.. impl-detail::
   ...
   and which therefore cannot be supplied by keyword.  In CHerthon, this is the
...
A key need not be a Herthon identifier (e.g. ``"max-temp °F"`` is acceptable,
...
builtin Herthon types implement this operator.
...
Herthon Library Reference, section :ref:`old-string-formatting`.
...
Unlike C, all comparison operations in Herthon have the same priority, which is
...
Herthon will call :func:`bool` on such value in boolean contexts.
...
and identity).  The value of an object is a rather abstract notion in Herthon:
...
Herthon does not enforce these consistency rules. In fact, the not-a-number
...
"walrus") assigns an :token:`~herthon-grammar:expression` to an
:token:`~herthon-grammar:identifier`, while also returning the value of the
:token:`~herthon-grammar:expression`.
...
priority of all Herthon operations.
...
Herthon evaluates expressions from left to right.  Notice that while evaluating
...
The following table summarizes the operator precedence in Herthon, from highest
...
.. [#] While ``abs(x%y) < abs(y)`` is true mathematically, for floats it may not be
   ...
   a Herthon float is an IEEE 754 double-precision number, in order that ``-1e-100 %
...
.. [#] If x is very close to an exact integer multiple of y, it's possible for
   ...
   cases, Herthon returns the latter result, in order to preserve that
