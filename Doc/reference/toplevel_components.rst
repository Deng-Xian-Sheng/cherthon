...
The Herthon interpreter can get its input from a number of sources: from a script
...
is invoked, it is useful to have a notion of a complete Herthon program.  A
complete Herthon program is executed in a minimally initialized environment: all
...
The syntax for a complete Herthon program is that for file input, described in
the next section.
...
.. grammar-snippet::
   :group: herthon-grammar

   file_input: (NEWLINE | `statement`)* ENDMARKER

This syntax is used in the following situations:

* when parsing a complete Herthon program (from a file or from a string);
