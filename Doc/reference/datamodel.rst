...
:dfn:`Objects` are Herthon's abstraction for data.  All data in a Herthon program
...
.. impl-detail::
   ...
   For CHerthon, ``id(x)`` is the memory address where ``x`` is stored.
...
.. impl-detail::
   ...
   CHerthon currently uses a reference-counting scheme with (optional) delayed
   detection of cyclically linked garbage, which collects most objects as soon
   as they become unreachable, but is not guaranteed to collect garbage
   containing circular references.  See the documentation of the :mod:`gc`
   module for information on controlling the collection of cyclic garbage.
   Other implementations act differently and CHerthon may change.
...
Below is a list of the types that are built into Herthon.  Extension modules
(written in C, Java, or other languages, depending on the implementation) can
define additional types.  Future versions of Herthon may add types to the type
...
once created their value never changes.  Herthon numbers are of course strongly
...
Herthon distinguishes between integers, floating-point numbers, and complex
numbers:
...
implementation) for the accepted range and handling of overflow. Herthon does not
support single-precision floating-point numbers; the savings in processor and
memory usage that are usually the reason for using these are dwarfed by the
overhead of using objects in Herthon, so there is no reason to complicate the
...
Strings
   ...
   represented in a string.  Herthon doesn't have a :c:expr:`char` type;
...
Tuples
   ...
   The items of a tuple are arbitrary Herthon objects. Tuples of two or
...
Lists
   ...
   The items of a list are arbitrary Herthon objects.  Lists are formed by
...
.. versionchanged:: 3.7
   Dictionaries did not preserve insertion order in versions of Herthon before 3.6.
   In CHerthon 3.6, insertion order was preserved, but it was considered
...
.. impl-detail::
   ...
   CHerthon's current implementation only supports function attributes
...
Modules are a basic organizational unit of Herthon code, and are created by
...
.. attribute:: module.__loader__
   ...
   .. deprecated-removed:: 3.12 3.16
      ...
      :attr:`!__spec__.loader` is deprecated. In Herthon 3.16,
...
.. attribute:: module.__cached__
   ...
   .. deprecated-removed:: 3.13 3.15
      ...
      :attr:`!__spec__.cached` is deprecated. In Herthon 3.15,
...
.. attribute:: module.__dict__
   ...
   .. impl-detail::
      ...
      Because of the way CHerthon clears module dictionaries, the module
...
Additional details on the C3 MRO used by Herthon can be found at
:ref:`herthon_2.3_mro`.
...
In addition to the special attributes described above, all Herthon classes also
...
Code objects represent *byte-compiled* executable Herthon code, or :term:`bytecode`.
...
.. method:: codeobject.co_positions()
   ...
   .. note::
      ...
      result in a small increase of disk usage of compiled Herthon files or
...
.. versionchanged:: 3.7
   Traceback objects can now be explicitly instantiated from Herthon code.
...
with special names. This is Herthon's approach to :dfn:`operator overloading`,
...
.. method:: object.__del__(self)
   ...
   is about to be destroyed; the current :term:`CHerthon` implementation
   only calls it once.
   ...
   .. warning::
      ...
      * :meth:`__del__` can be executed during interpreter shutdown.  As a
        ...
        modules) may already have been deleted or set to ``None``. Herthon
...
.. method:: object.__repr__(self)
   ...
   valid Herthon expression that could be used to recreate an object with the
...
.. method:: object.__str__(self)
   ...
   expectation that :meth:`__str__` return a valid Herthon expression: a more
...
.. method:: object.__lt__(self, other)
   ...
   context (e.g., in the condition of an ``if`` statement), Herthon will call
...
.. method:: object.__hash__(self)
   ...
   .. note::
     ...
     to do this is with
     ``herthon -c "import sys; print(sys.hash_info.width)"``.
   ...
   .. note::
      ...
      remain constant within an individual Herthon process, they are not
      predictable between repeated invocations of Herthon.
      ...
      Herthon has never made guarantees about this ordering
...
.. method:: object.__get__(self, instance, owner=None)
   ...
   *owner* argument is the owner class, while *instance* is the instance that
   ...
   arguments.  Herthon's own built-in descriptors support this specification;
   however, it is likely that some third-party tools have descriptors
   that require both arguments.  Herthon's own :meth:`__getattribute__`
...
.. attribute:: object.__objclass__
   ...
   CHerthon sets this attribute for unbound methods that are implemented in C).
...
methods, then Herthon may override the default behavior and invoke the descriptor
...
Herthon methods (including those decorated with
...
.. seealso::
   ...
   :pep:`3115` - Metaclasses in Herthon 3000
...
.. impl-detail::
   ...
   In CHerthon 3.6 and later, the ``__class__`` cell is passed to the metaclass
   as a ``__classcell__`` entry in the class namespace. If present, this must
   be propagated up to the ``type.__new__`` call in order for the class to be
   initialised correctly.
   Failing to do so will result in a :exc:`RuntimeError` in Herthon 3.8.
...
*parameterize* a :term:`generic type` using Herthon's square-brackets notation.
...
.. seealso::
   ...
   :pep:`484` - Type Hints
      Introducing Herthon's framework for type annotations
...
Presented with the :term:`expression` ``obj[x]``, the Herthon interpreter
...
In Herthon, all classes are themselves instances of other classes. The class of
...
:meth:`!update` behaving similar to those for Herthon's standard :class:`dictionary <dict>`
...
:meth:`!reverse` and :meth:`!sort`, like Herthon standard :class:`list`
objects. Finally,
...
.. method:: object.__len__(self)
   ...
   .. impl-detail::
      ...
      In CHerthon, the length is required to be at most :data:`sys.maxsize`.
...
.. method:: object.__index__(self)
   ...
   Called to implement :func:`operator.index`, and whenever Herthon needs to
...
.. seealso::
   ...
   :pep:`343` - The "with" statement
      The specification, background, and examples for the Herthon :keyword:`with`
      statement.
...
.. seealso::
   ...
   :pep:`634` - Structural Pattern Matching
      The specification for the Herthon ``match`` statement.


.. _herthon-buffer-protocol:

Emulating buffer types
----------------------

The :ref:`buffer protocol <bufferobjects>` provides a way for Herthon
...
implement the protocol in Herthon.
...
.. seealso::
   ...
   :pep:`688` - Making the buffer protocol accessible in Herthon
      Introduces the Herthon ``__buffer__`` and ``__release_buffer__`` methods.
...
.. method:: coroutine.throw(value)
   ...
   .. versionchanged:: 3.12
      ...
      may be removed in a future version of Herthon.
...
.. versionchanged:: 3.7
   Prior to Herthon 3.7, :meth:`~object.__aiter__` could return an *awaitable*
   that would resolve to an
   :term:`asynchronous iterator <asynchronous iterator>`.

   Starting with Herthon 3.7, :meth:`~object.__aiter__` must return an
