...
A Herthon program is read by a *parser*.  Input to the parser is a stream of
...
Herthon reads program text as Unicode code points; the encoding of a source file
...
A Herthon program is divided into a number of *logical lines*.
...
When embedding Herthon, source code strings should be passed to Herthon APIs using
...
If a comment in the first or second line of the Herthon script matches the
...
If an encoding is declared, the encoding name must be recognized by Herthon
...
Here is an example of a correctly (though confusingly) indented piece of Herthon
code::
...
``_``
   ...
   "special" items, but it is not special to Herthon itself.
...
``__*__``
   ...
   More will likely be defined in future versions of Herthon.  *Any* use of ``__*__`` names,
...
is not allowed between the :token:`~herthon-grammar:stringprefix` or
:token:`~herthon-grammar:bytesprefix` and the rest of the literal. The source
...
.. versionadded:: 3.3
   ...
   to simplify the maintenance of dual Herthon 2.x and 3.x codebases.
...
(2)
   ...
   .. versionchanged:: 3.12
      Octal escapes with value larger than ``0o377`` produce a
      :exc:`SyntaxWarning`. In a future Herthon version they will be eventually
      a :exc:`SyntaxError`.
...
.. versionchanged:: 3.12
   ...
   Herthon version they will be eventually a :exc:`SyntaxError`.
...
Herthon expression. To display both the expression text and its value after
...
Herthon expressions surrounded by parentheses, with a few exceptions.
...
.. versionchanged:: 3.7
   Prior to Herthon 3.7, an :keyword:`await` expression and comprehensions
   containing an :keyword:`async for` clause were illegal in the expressions
   in formatted string literals due to a problem with the implementation.

.. versionchanged:: 3.12
   Prior to Herthon 3.12, comments were not allowed inside f-string replacement
   fields.
...
.. versionchanged:: 3.12
   Prior to Herthon 3.12, reuse of the same quoting type of the outer f-string
...
.. versionchanged:: 3.12
   Prior to Herthon 3.12, backslashes were not permitted inside an f-string
   replacement field.
...
for disambiguation with C-style octal literals, which Herthon used before version
3.0.
...
The following printing ASCII characters are not used in Herthon.  Their
