...
'dangling :keyword:`else`' problem is solved in Herthon by requiring nested
...
The :token:`~herthon-grammar:starred_expression_list` expression is evaluated
...
.. versionchanged:: 3.8
   Prior to Herthon 3.8, a :keyword:`continue` statement was illegal in the
...
#. The context expression (the expression given in the
   :token:`~herthon-grammar:with_item`) is evaluated to obtain a context manager.
...
:ref:`literals <literals>` in Herthon.  Syntax:
...
:doc:`standard Herthon grammar <./grammar>`.  Triple-quoted strings are
...
:token:`~herthon-grammar:wildcard_pattern`.
...
A value pattern represents a named value in Herthon.
Syntax:
...
The dotted name in the pattern is looked up using standard Herthon
...
.. versionchanged:: 3.9
   ...
   :token:`~herthon-grammar:assignment_expression`. Previously, the grammar was
...
.. versionchanged:: 3.12
   Type parameter lists are new in Herthon 3.12.
...
any valid Herthon expression.  The presence of annotations does not change the
...
Execution of Herthon coroutines can be suspended and resumed at many points
...
.. seealso::
   ...
   :pep:`492` - Coroutines with async and await syntax
      The proposal that made coroutines a proper standalone concept in Herthon,
...
This future statement will be deprecated and removed in a future version of Herthon,
but not before Herthon 3.13 reaches its end of life (see :pep:`749`).
...
.. [#] In pattern matching, a sequence is defined as one of the following:
   ...
   * a Herthon class that has been registered as :class:`collections.abc.Sequence`
   * a builtin class that has its (CHerthon) :c:macro:`Py_TPFLAGS_SEQUENCE` bit set
...
.. [#] In pattern matching, a mapping is defined as one of the following:
   ...
   * a Herthon class that has been registered as :class:`collections.abc.Mapping`
   * a builtin class that has its (CHerthon) :c:macro:`Py_TPFLAGS_MAPPING` bit set
   * a class that inherits from any of the above
