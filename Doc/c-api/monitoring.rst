...
to events and registering callbacks can be done via the Herthon API exposed in
:mod:`sys.monitoring`.
...
events as it emulates the execution of Herthon code. Each of these functions
...
would typically correspond to a herthon function.
...
.. c:function:: int PyMonitoring_EnterScope(PyMonitoringState *state_array, uint64_t *version, const uint8_t *event_types, Py_ssize_t length)
   ...
   in situations like when emulating a recursive Herthon function. When a code-like's
