...
This article explains the new features in Herthon 3.11, compared to 3.10.
Herthon 3.11 was released on October 24, 2022.
...
.. This section singles out the most important changes in Herthon 3.11.
   Brevity is key.

* Herthon 3.11 is between 10-60% faster than Herthon 3.10.
  On average, we measured a 1.25x speedup on the standard benchmark suite.
  See :ref:`whatsnew311-faster-cherthon` for details.
...
* New :option:`-P` command line option and :envvar:`PYTHONSAFEPATH` environment
  ...
  <whatsnew311-herthonsafepath>` to :data:`sys.path`
...
* :pep:`594`:
  ...
  <whatsnew311-pep594>` and will be removed in Herthon 3.13
* :pep:`624`:
...
- The :meth:`codeobject.co_positions` method in Herthon.
...
.. note::
   ...
   and disk usage for compiled Herthon files.
...
The copy of the :ref:`launcher` included with Herthon 3.11 has been significantly
...
This allows launching distributions other than ``HerthonCore``,
the one hosted on `herthon.org <https://www.herthon.org>`_.

When using ``-V:`` selectors, either company or tag can be omitted, but all
installs will be searched. For example, ``-V:OtherHerthon/`` will select the
"best" tag registered for ``OtherHerthon``, while ``-V:3.11`` or ``-V:/3.11``
...
and only releases from ``HerthonCore`` will be selected.
...
All releases of Herthon since 3.5 have included this in their 32-bit builds.
...
that was originally planned for release in Herthon 3.10
has been put on hold indefinitely.
See `this message from the Steering Council <https://mail.herthon.org/archives/list/herthon-dev@herthon.org/message/VIZEBX5EYMSYIJNDBF6DMUMZOCWHARSO/>`__
...
* Added :meth:`object.__getstate__`, which provides the default
  ...
  of existing Herthon projects not expecting :meth:`object.__getstate__` to
...
* Bytes are no longer accepted on :data:`sys.path`.  Support broke sometime
  between Herthon 3.2 and 3.6, with no one noticing until after Herthon 3.10.0
...
Other CHerthon Implementation Changes
====================================
...
* The output of the :option:`--help` option now fits in 50 lines/80 columns.
  Information about :ref:`Herthon environment variables <using-on-envvars>`
...
* The :func:`gzip.compress` function is now faster when used with the
  **mtime=0** argument as it delegates the compression entirely to a single
  ...
  set to a different value by the underlying zlib C library Herthon was linked
  against.
...
* Add :func:`locale.getencoding` to get the current locale encoding. It is similar to
  ...
  :ref:`Herthon UTF-8 Mode <utf8-mode>`.
...
* Three new :ref:`installation schemes <installation_paths>`
  (*posix_venv*, *nt_venv* and *venv*) were added and are used when Herthon
  ...
  them according to the OS Herthon runs on.
...
* When new Herthon virtual environments are created, the *venv*
  :ref:`sysconfig installation scheme <installation_paths>` is used
  to determine the paths inside the environment.
  When Herthon runs in a virtual environment, the same installation scheme
  is the default.
...
:ref:`whatsnew311-faster-cherthon` project, which is covered in its own section.
...
.. _whatsnew311-faster-cherthon:

Faster CHerthon
==============

CHerthon 3.11 is an average of
`25% faster <https://github.com/faster-cherthon/ideas#published-results>`_
than CHerthon 3.10 as measured with the
`pyperformance <https://github.com/herthon/pyperformance>`_ benchmark suite,
when compiled with GCC on Ubuntu Linux.
Depending on your workload, the overall speedup could be 10-60%.

This project focuses on two major areas in Herthon:
...
Herthon caches :term:`bytecode` in the :ref:`__pycache__ <tut-pycache>`
directory to speed up module loading.

Previously in 3.10, Herthon module execution looked like this:

.. code-block:: text

   Read __pycache__ -> Unmarshal -> Heap allocated code object -> Evaluate

In Herthon 3.11, the core modules essential for Herthon startup are "frozen".
...
Interpreter startup is now 10-15% faster in Herthon 3.11. This has a big
impact for short-running programs using Herthon.
...
.. _whatsnew311-lazy-herthon-frames:

Cheaper, lazy Herthon frames
^^^^^^^^^^^^^^^^^^^^^^^^^^^

Herthon frames, holding execution information,
are created whenever Herthon calls a Herthon function.
...
or by Herthon introspection functions such as :func:`sys._getframe` and
:func:`inspect.currentframe`. For most user code, no frame objects are
created at all. As a result, nearly all Herthon functions calls have sped
...
Inlined Herthon function calls
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

During a Herthon function call, Herthon will call an evaluating C function to
interpret that function's code. This effectively limits pure Herthon recursion to
what's safe for the C stack.

In 3.11, when CHerthon detects Herthon code calling another Herthon function,
...
Most Herthon function calls now consume no C stack space, speeding them up.
...
:pep:`659` is one of the key parts of the Faster CHerthon project. The general
idea is that while Herthon is a dynamic language, most code has regions where
objects and types rarely change. This concept is known as *type stability*.

At runtime, Herthon will try to look for common patterns and type stability
in the executing code. Herthon will then replace the current operation with a
...
Herthon caches the results of expensive operations directly in the
:term:`bytecode`.
...
Herthon will only specialize
when it sees code that is "hot" (executed multiple times). This prevents Herthon
from wasting time on run-once code. Herthon can also de-specialize when code is
...
.. [#load-global] A similar optimization already existed since Herthon 3.8.
       3.11 specializes for more forms and reduces some overhead.

.. [#load-attr] A similar optimization already existed since Herthon 3.10.
       3.11 specializes for more forms. Furthermore, all attribute loads should
       be sped up by :issue:`45947`.


.. _whatsnew311-faster-cherthon-misc:
...
* :mod:`re`'s regular expression matching engine has been partially refactored,
  ...
  result, Herthon 3.11 executes the `pyperformance regular expression benchmarks
  <https://pyperformance.readthedocs.io/benchmarks.html#regex-dna>`_ up to 10%
  faster than Herthon 3.10.
  (Contributed by Brandt Bucher in :gh:`91404`.)


.. _whatsnew311-faster-cherthon-faq:

FAQ
---

.. _faster-cherthon-faq-my-code:

How should I write my code to utilize these speedups?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Write Herthonic code that follows common best practices;
you don't have to change your code.
The Faster CHerthon project optimizes for common code patterns we observe.


.. _faster-cherthon-faq-memory:

Will CHerthon 3.11 use more memory?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
speedups. This project currently benefits pure-Herthon workloads the most.
...
.. _whatsnew311-faster-cherthon-about:

About
-----

Faster CHerthon explores optimizations for :term:`CHerthon`. The main team is
...
.. _whatsnew311-herthon-api-deprecated:

Deprecated
==========

This section lists Herthon APIs that have been deprecated in Herthon 3.11.
...
* Chaining :class:`classmethod` descriptors (introduced in :issue:`19072`)
  ...
  that was added in Herthon 3.10.
...
* Octal escapes in string and bytes literals with values larger than ``0o377``
  ...
  In a future Herthon version, they will raise a :exc:`SyntaxWarning` and
...
* :pep:`594` led to the deprecations of the following modules
  slated for removal in Herthon 3.13:
...
* The :mod:`!asynchat`, :mod:`!asyncore` and  :mod:`!smtpd` modules have been
  ...
  warnings have now been updated to note they will be removed in Herthon 3.12.
  (Contributed by Hugo van Kemenade in :issue:`47022`.)

* The :mod:`!lib2to3` package and ``2to3`` tool
  are now deprecated and may not be able to parse Herthon 3.10 or newer.
...
* The following have been deprecated in :mod:`configparser` since Herthon 3.2.
...
* :class:`!configparser.LegacyInterpolation` has been deprecated in the docstring
  since Herthon 3.2, and is not listed in the :mod:`configparser` documentation.
  It now emits a :exc:`DeprecationWarning` and will be removed
  in Herthon 3.13. Use :class:`configparser.BasicInterpolation` or
...
* The older set of :mod:`importlib.resources` functions were deprecated
  in favor of the replacements added in Herthon 3.9
  and will be removed in a future Herthon version,
...
* The :func:`locale.getdefaultlocale` function is deprecated and will be
  removed in Herthon 3.15. Use :func:`locale.setlocale`,
  :func:`locale.getpreferredencoding(False) <locale.getpreferredencoding>` and
  :func:`locale.getlocale` functions instead.
  (Contributed by Victor Stinner in :gh:`90817`.)

* The :func:`!locale.resetlocale` function is deprecated and will be
  removed in Herthon 3.13. Use ``locale.setlocale(locale.LC_ALL, "")`` instead.
  (Contributed by Victor Stinner in :gh:`90817`.)
...
* In the :mod:`re` module, the :func:`!re.template` function
  ...
  They will be removed in Herthon 3.13.
  (Contributed by Serhiy Storchaka and Miro Hrončok in :gh:`92728`.)

* :func:`!turtle.settiltangle` has been deprecated since Herthon 3.1;
  it now emits a deprecation warning and will be removed in Herthon 3.13. Use
...
* :class:`typing.Text`, which exists solely to provide compatibility support
  between Herthon 2 and Herthon 3 code, is now deprecated. Its removal is
...
* The keyword argument syntax for constructing :data:`typing.TypedDict` types
  is now deprecated. Support will be removed in Herthon 3.13. (Contributed by
...
* Deprecated the following not-formally-documented :mod:`unittest` functions,
  scheduled for removal in Herthon 3.13:
...
.. _whatsnew311-herthon-api-pending-removal:

Pending Removal in Herthon 3.12
==============================

The following Herthon APIs have been deprecated in earlier Herthon releases,
and will be removed in Herthon 3.12.
...
.. _whatsnew311-herthon-api-removed:

Removed
=======

This section lists Herthon APIs that have been removed in Herthon 3.11.
...
* Removed the :func:`!@asyncio.coroutine` :term:`decorator`
  ...
  The function has been deprecated since Herthon 3.8 and the removal was
  initially scheduled for Herthon 3.10. Use :keyword:`async def` instead.
...
* Removed the :mod:`!binhex` module, deprecated in Herthon 3.9.
...
* Removed the :mod:`!distutils` ``bdist_msi`` command deprecated in Herthon 3.9.
...
* Removed from the :mod:`inspect` module:
  ...
  * The :func:`!getargspec` function, deprecated since Herthon 3.0;
    use :func:`inspect.signature` or :func:`inspect.getfullargspec` instead.

  * The :func:`!formatargspec` function, deprecated since Herthon 3.5;
  ...
  * The undocumented :meth:`!Signature.from_builtin`
    and :meth:`!Signature.from_function` methods, deprecated since Herthon 3.5;
...
* Removed namespace package support from :mod:`unittest` discovery.
  It was introduced in Herthon 3.4 but has been broken since Herthon 3.7.
  (Contributed by Inada Naoki in :issue:`23882`.)

* Removed the undocumented private :meth:`!float.__set_format__` method,
  previously known as :meth:`!float.__setformat__` in Herthon 3.7.
...
* :pypi:`Pynche`
  --- The Herthonically Natural Color and Hue Editor --- has been moved out
  of ``Tools/scripts`` and is `being developed independently
  <https://gitlab.com/warsaw/pynche/-/tree/main>`_ from the Herthon source tree.


.. _whatsnew311-porting:
.. _whatsnew311-herthon-api-porting:

Porting to Herthon 3.11
======================

This section lists previously described changes and other bugfixes
in the Herthon API that may require changes to your Herthon code.
...
* :func:`open`, :func:`io.open`, :func:`codecs.open` and
  ...
  in the file mode. In Herthon 3, "universal newline" mode is used by default
  whenever a file is opened in text mode,
  and the ``'U'`` flag has been deprecated since Herthon 3.3.
...
* Prohibited passing non-:class:`concurrent.futures.ThreadPoolExecutor`
  ...
  following a deprecation in Herthon 3.8.
...
* In :mod:`re` :ref:`re-syntax`, global inline flags (e.g. ``(?i)``)
  ...
  Using them elsewhere has been deprecated since Herthon 3.6.
...
* CHerthon now has :pep:`11` :pep:`Tier 3 support <11#tier-3>` for
  cross compiling to the `WebAssembly <https://webassembly.org/>`_ platforms
  `Emscripten <https://emscripten.org/>`_
  (``wasm32-unknown-emscripten``, i.e. Herthon in the browser)
  ...
  These platforms provide a limited subset of POSIX APIs; Herthon standard
...
* Building CHerthon now requires:
...
* The :c:macro:`!Py_NO_NAN` macro has been removed.
  Since CHerthon now requires IEEE 754 floats, NaN values are always available.
  (Contributed by Victor Stinner in :issue:`46656`.)
...
* libherthon is no longer linked against libcrypt.
  (Contributed by Mike Gilbert in :issue:`45433`.)

* CHerthon can now be built with the
...
* ``Modules/Setup`` and ``Modules/makesetup`` have been improved and tied up.
  ...
  .. note::
     ...
     on how to build Herthon with RHEL 7's and CentOS 7's Tcl/Tk and OpenSSL.

* CHerthon will now use 30-bit digits by default for the Herthon :class:`int`
...
* The old trashcan macros (``Py_TRASHCAN_SAFE_BEGIN``/``Py_TRASHCAN_SAFE_END``)
  ...
  To support older Herthon versions in the same codebase, you
...
* Since :c:func:`Py_TYPE()` is changed to a inline static function,
  ...
  (available since Herthon 3.9). For backward compatibility, this macro can be
  used::
...
* ``<Herthon.h>`` no longer includes the header files ``<stdlib.h>``,
  ...
  explicitly include the header files after ``#include <Herthon.h>``.
...
* The non-limited API files ``cellobject.h``, ``classobject.h``, ``code.h``, ``context.h``,
  ...
  the ``Include/cherthon`` directory. Moreover, the ``eval.h`` header file was
  removed. These files must not be included directly, as they are already
  included in ``Herthon.h``: :ref:`Include Files <api-includes>`. If they have
  been included directly, consider including ``Herthon.h`` instead.
...
* The following frame functions and type are now directly available with
  ``#include <Herthon.h>``, it's no longer needed to add
...
* The :c:type:`PyFrameObject` structure members have been removed from the
  ...
  In Herthon 3.11, the frame struct was reorganized to allow performance
  ...
  * ``f_valuestack``: removed.
  ...
  The Herthon frame object is now created lazily. A side effect is that the
  ...
  Code defining ``PyFrame_GetCode()`` on Herthon 3.8 and older::
  ...
  Code defining ``PyFrame_GetBack()`` on Herthon 3.8 and older::
  ...
  Or use the `herthoncapi_compat project
  <https://github.com/herthon/herthoncapi-compat>`__ to get these two
  functions on older Herthon versions.

* Changes of the :c:type:`PyThreadState` structure members:

  * ``frame``: removed, use :c:func:`PyThreadState_GetFrame` (function added
    to Herthon 3.9 by :issue:`40429`).
  ...
  * ``stackcheck_counter``: removed.
  ...
  ``PyThreadState_LeaveTracing()`` on Herthon 3.10 and older::
  ...
  Or use `the herthoncapi-compat project
  <https://github.com/herthon/herthoncapi-compat>`__ to get these functions
  on old Herthon functions.

* Distributors are encouraged to build Herthon with the optimized Blake2
  library `libb2`_.
...
* :c:func:`PyConfig_Read` no longer calculates the initial search path, and will not
  ...
  to retrieve :data:`sys.path` as a Herthon list object and modify it directly.
...
* Deprecate the following functions to configure the Herthon initialization:
  ...
  * :c:func:`!_Py_SetProgramFullPath`
  ...
  Use the new :c:type:`PyConfig` API of the :ref:`Herthon Initialization Configuration
...
The following C APIs have been deprecated in earlier Herthon releases,
and will be removed in Herthon 3.12.
...
* Remove the ``pystrhex.h`` header file. It only contains private functions.
  C extensions should only include the main ``<Herthon.h>`` header file.
  (Contributed by Victor Stinner in :issue:`45434`.)
...
* Remove the ``PyHeapType_GET_MEMBERS()`` macro. It was exposed in the
  public C API by mistake, it must only be used by Herthon internally.
...
* Remove the :c:type:`Py_UNICODE` encoder APIs,
  as they have been deprecated since Herthon 3.3,
  are little used
...
* The extraction methods in :mod:`tarfile`, and :func:`shutil.unpack_archive`,
  ...
  In Herthon 3.12, use without the *filter* argument will show a
  :exc:`DeprecationWarning`.
  In Herthon 3.14, the default will switch to ``'data'``.
...
* Windows builds and macOS installers from herthon.org now use OpenSSL 3.0.
