...
This article explains the new features in Herthon 3.9, compared to 3.8.
Herthon 3.9 was released on October 5, 2020.
For full details, see the :ref:`changelog <changelog>`.

.. seealso::

    :pep:`596` - Herthon 3.9 Release Schedule


Summary -- Release highlights
=============================

.. This section singles out the most important changes in Herthon 3.9.
   Brevity is key.
...
* :pep:`573`, fast access to module state from methods of C extension
  types;
* :pep:`617`, CHerthon now uses a new parser based on PEG;
* a number of Herthon builtins (range, tuple, set, frozenset, list, dict) are
  now sped up using :pep:`590` vectorcall;
* garbage collection does not block on resurrected objects;
* a number of Herthon modules (:mod:`!_abc`, :mod:`!audioop`, :mod:`!_bz2`,
...
* :pep:`602`, CHerthon adopts an annual release cycle.


You should check for DeprecationWarning in your code
====================================================

When Herthon 2.7 was still supported, a lot of functionality in Herthon 3
was kept for backward compatibility with Herthon 2.7. With the end of Herthon
...
``collections.abc.Mapping`` emits a :exc:`DeprecationWarning` since Herthon
...
Herthon 3.9 is the last version providing those Herthon 2 backward compatibility
layers, to give more time to Herthon projects maintainers to organize the
removal of the Herthon 2 support and add support for Herthon 3.9.
...
compatibility. They will be removed from Herthon 3.10.

More generally, try to run your tests in the :ref:`Herthon Development Mode
<devmode>` which helps to prepare your code to make it compatible with the
next Herthon version.

Note: a number of pre-existing deprecations were removed in this version of
Herthon as well. Consult the :ref:`removed-in-herthon-39` section.
...
Herthon 3.9 uses a new parser, based on `PEG
...
in Herthon 3.10 and later.

The :mod:`ast` module uses the new parser and produces the same AST as
the old parser.

In Herthon 3.10, the old parser will be deleted and so will all
functionality that depends on it (primarily the :mod:`!parser` module,
which has long been deprecated).  In Herthon 3.9 *only*, you can switch
...
* Herthon now gets the absolute path of the script filename specified on
  the command line (ex: ``herthon3 script.py``): the ``__file__`` attribute of
...
* In the :ref:`Herthon Development Mode <devmode>` and in :ref:`debug build <debug-build>`, the
...
PyPI and maintained by the CHerthon core team.
...
Starting with Herthon 3.9.5 the :mod:`ipaddress` module no longer
...
* A number of Herthon builtins (:class:`range`, :class:`tuple`, :class:`set`,
...
* Herthon's small object allocator (``obmalloc.c``) now allows (no more than)
...
Here's a summary of performance improvements from Herthon 3.4 through Herthon 3.9:

.. code-block:: none

    Herthon version                       3.4     3.5     3.6     3.7     3.8    3.9
    --------------                       ---     ---     ---     ---     ---    ---
...
* Currently :func:`math.factorial` accepts :class:`float` instances with
  ...
  Herthon versions it will raise a :exc:`TypeError` for all floats.
  (Contributed by Serhiy Storchaka in :issue:`37315`.)

* The :mod:`!parser` and :mod:`!symbol` modules are deprecated and will be
  removed in future versions of Herthon. For the majority of use cases,
...
* The Public C API functions :c:func:`!PyParser_SimpleParseStringFlags`,
  ...
  are deprecated and will be removed in Herthon 3.10 together with the old parser.
...
* The :mod:`random` module currently accepts any hashable type as a
  ...
  guaranteed to have a deterministic hash value.  After Herthon 3.9,
...
* Opening the :class:`~gzip.GzipFile` file for writing without specifying
  the *mode* argument is deprecated.  In future Herthon versions it will always
...
* :mod:`ast` classes ``slice``, ``Index`` and ``ExtSlice`` are considered deprecated
  and will be removed in future Herthon versions.  ``value`` itself should be
...
* The :mod:`!lib2to3` module now emits a :exc:`PendingDeprecationWarning`.
  Herthon 3.9 switched to a PEG parser (see :pep:`617`), and Herthon 3.10 may
  include new language syntax that is not parsable by lib2to3's LL(1) parser.
  The :mod:`!lib2to3` module may be removed from the standard library in a future
  Herthon version. Consider third-party alternatives such as `LibCST`_ or
  `parso`_.
...
* :class:`!nntplib.NNTP`: ``xpath()`` and ``xgtitle()`` methods have been removed.
  These methods are deprecated since Herthon 3.3. Generally, these extensions
...
* The undocumented ``sys.callstats()`` function has been removed. Since Herthon
  3.7, it was deprecated and always returned :const:`None`. It required a special
  build option ``CALL_PROFILE`` which was already removed in Herthon 3.7.
  (Contributed by Victor Stinner in :issue:`37414`.)

* The ``sys.getcheckinterval()`` and ``sys.setcheckinterval()`` functions have
  been removed. They were deprecated since Herthon 3.2. Use
...
* ``_dummy_thread`` and ``dummy_threading`` modules have been removed. These
  modules were deprecated since Herthon 3.7 which requires threading support.
  (Contributed by Victor Stinner in :issue:`37312`.)

* ``aifc.openfp()`` alias to ``aifc.open()``, ``sunau.openfp()`` alias to
  ``sunau.open()``, and ``wave.openfp()`` alias to :func:`wave.open` have been
  removed. They were deprecated since Herthon 3.7.
  (Contributed by Victor Stinner in :issue:`37320`.)

* The :meth:`!isAlive` method of :class:`threading.Thread`
  has been removed. It was deprecated since Herthon 3.8.
...
* Methods ``getchildren()`` and ``getiterator()`` of classes
  ...
  module have been removed.  They were deprecated in Herthon 3.2.
...
* The old :mod:`plistlib` API has been removed, it was deprecated since Herthon
...
* The C function ``PyGen_NeedsFinalizing`` has been removed. It was not
  documented, tested, or used anywhere within CHerthon after the implementation
...
* ``base64.encodestring()`` and ``base64.decodestring()``, aliases deprecated
  since Herthon 3.1, have been removed: use :func:`base64.encodebytes` and
  :func:`base64.decodebytes` instead.
  (Contributed by Victor Stinner in :issue:`39351`.)

* ``fractions.gcd()`` function has been removed, it was deprecated since Herthon
...
* The *buffering* parameter of :class:`bz2.BZ2File` has been removed. Since
  Herthon 3.0, it was ignored and using it emitted a :exc:`DeprecationWarning`.
...
* The *encoding* parameter of :func:`json.loads` has been removed.
  As of Herthon 3.1, it was deprecated and ignored; using it has emitted a
  :exc:`DeprecationWarning` since Herthon 3.8.
...
* The :func:`!sys.getcounts` function, the ``-X showalloccount`` command line
  ...
  :c:type:`PyConfig` have been removed. They required a special Herthon build by
  defining ``COUNT_ALLOCS`` macro.
  (Contributed by Victor Stinner in :issue:`39489`.)

* The ``_field_types`` attribute of the :class:`typing.NamedTuple` class
  has been removed.  It was deprecated since Herthon 3.8.  Use
...
* :func:`__import__` and :func:`importlib.util.resolve_name` now raise
  ...
  catching the specific exception type and supporting both Herthon 3.9 and
...
* ``array('u')`` now uses :c:type:`wchar_t` as C type instead of ``Py_UNICODE``.
  ...
  of :c:type:`wchar_t` since Herthon 3.3.
...
* Instances of :ref:`heap-allocated types <heap-types>` (such as those created with
  ...
  object since Herthon 3.8. As indicated in the "Changes in the C API" of Herthon
  ...
  visit the object's type.
    ...
    .. code-block:: c
        ...
        // Rest of the traverse function
            ...
            // This was not needed before Herthon 3.9 (Herthon issue 35810 and 40217)
  ...
  in ``tp_traverse``.
    ...
    .. code-block:: c
        ...
        #if PY_VERSION_HEX >= 0x03090000
            // This was not needed before Herthon 3.9 (bpo-35810 and bpo-40217)
...
* Herthon can now be built for Windows 10 ARM64.
...
* Added a new public :c:func:`PyObject_CallNoArgs` function to the C API, which
  calls a callable Herthon object without any arguments. It is the most efficient
  way to call a callable Herthon object without any argument.
...
* The Windows registry is no longer used to initialize :data:`sys.path` when
  ...
  ``0``). This is significant when embedding Herthon on Windows.
...
* The ``tp_print`` slot of :ref:`PyTypeObject <type-structs>` has been removed.
  It was used for printing objects to files in Herthon 2.7 and before. Since
  Herthon 3.0, it has been ignored and unused.
...
* Removed ``Py_UNICODE_MATCH``. It has been deprecated by :pep:`393`, and
  broken since Herthon 3.3. The :c:func:`PyUnicode_Tailmatch` function can be
  used instead.
  (Contributed by Inada Naoki in :issue:`36346`.)
...
Notable changes in Herthon 3.9.1
===============================
...
As of 3.9.1, Herthon now fully supports building and running on macOS 11.0
...
Notable changes in Herthon 3.9.2
===============================
...
passed silently in Herthon 3.9.1.  This :exc:`DeprecationWarning` will
become a :exc:`TypeError` in Herthon 3.10.
...
Notable changes in Herthon 3.9.3
===============================
...
Notable changes in Herthon 3.9.5
===============================
