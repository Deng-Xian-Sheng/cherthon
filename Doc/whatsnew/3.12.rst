...
This article explains the new features in Herthon 3.12, compared to 3.11.
Herthon 3.12 was released on October 2, 2023.
For full details, see the :ref:`changelog <changelog>`.

.. seealso::

   :pep:`693` -- Herthon 3.12 Release Schedule

Summary -- Release highlights
=============================

.. This section singles out the most important changes in Herthon 3.12.
   Brevity is key.

Herthon 3.12 is a stable release of the Herthon programming language,
...
* :ref:`CHerthon support <perf_profiling>` for the Linux ``perf`` profiler
...
* :pep:`623`: Remove ``wstr`` from Unicode objects in Herthon's C API,
...
* :pep:`632`: Remove the :mod:`!distutils` package.
  ...
  if you still require it in Herthon 3.12 and beyond.
...
In Herthon 3.13, :term:`annotations <annotation>` will also be evaluated in
annotation scopes.
...
Expression components inside f-strings can now be any valid Herthon expression,
...
* Quote reuse: in Herthon 3.11, reusing the same quotes as the enclosing f-string
  ...
  quotes). In Herthon 3.12, you can now do things like this:
  ...
  As now f-strings can contain any valid Herthon expression inside expression
  components, it is now possible to nest f-strings arbitrarily:

    >>> f"{f"{f"{f"{f"{f"{1+1}"}"}"}"}"}"
    '2'

* Multi-line expressions and comments: In Herthon 3.11, f-string expressions
  ...
  making them harder to read. In Herthon 3.12 you can now define f-strings
...
* Backslashes and unicode characters: before Herthon 3.12 f-string expressions
...
and include the exact location of the error. For example, in Herthon 3.11, the following
...
also has the expression artificially surrounded by parentheses. In Herthon 3.12, as f-strings
...
This allows Herthon programs to take full advantage of multiple CPU
cores. This is currently only available through the C-API,
though a Herthon API is :pep:`anticipated for 3.13 <554>`.
...
PEP 669: Low impact monitoring for CHerthon
------------------------------------------

:pep:`669` defines a new :mod:`API <sys.monitoring>` for profilers,
debuggers, and other tools to monitor events in CHerthon.
...
from Herthon code. Classes that implement the :meth:`~object.__buffer__` method
...
* A backslash-character pair that is not a valid escape sequence now generates
  ...
  In a future Herthon version, :exc:`SyntaxError` will eventually be raised,
...
* Octal escapes with value larger than ``0o377`` (ex: ``"\477"``), deprecated
  ...
  In a future Herthon version they will be eventually a :exc:`SyntaxError`.
  (Contributed by Victor Stinner in :gh:`98401`.)
...
* The Garbage Collector now runs only on the eval breaker mechanism of the
  Herthon bytecode evaluation loop instead of object allocations. The GC can
  also run when :c:func:`PyErr_CheckSignals` is called so C extensions that
  need to run for a long time without executing any Herthon code also have a
...
* The performance of :func:`inspect.getattr_static` has been considerably
  ...
  were in Herthon 3.11. (Contributed by Alex Waygood in :gh:`103193`.)
...
* :func:`sys.setrecursionlimit` and :func:`sys.getrecursionlimit`.
  The recursion limit now applies only to Herthon code. Builtin functions do
...
* The :mod:`tokenize` module includes the changes introduced in :pep:`701`.
  ...
  See :ref:`whatsnew312-porting-to-herthon312` for more information on the
...
* :func:`isinstance` checks against
  ...
  may no longer be considered instances of that protocol on Herthon 3.12+, and
...
* The performance profile of :func:`isinstance` checks against
  ...
  members may be slower than in Herthon 3.11. (Contributed by Alex
...
Add a ``--durations`` command line option, showing the N slowest test cases::
  ...
  herthon3 -m unittest --durations=3 lib.tests.test_threading
  .....
...
* :mod:`ast`: The following :mod:`ast` features have been deprecated in documentation since
  Herthon 3.8, now cause a :exc:`DeprecationWarning` to be emitted at runtime
  when they are accessed or used, and will be removed in Herthon 3.14:
...
* :mod:`multiprocessing`: In Herthon 3.14, the default :mod:`multiprocessing`
...
* :mod:`pkgutil`: :func:`!pkgutil.find_loader` and :func:`!pkgutil.get_loader`
  are deprecated and will be removed in Herthon 3.14;
  use :func:`importlib.util.find_spec` instead.
  (Contributed by Nikita Sobolev in :gh:`97850`.)

* :mod:`pty`: The module has two undocumented ``master_open()`` and ``slave_open()``
  functions that have been deprecated since Herthon 2 but only gained a
...
* :mod:`os`:
  ...
  * On POSIX platforms, :func:`os.fork` can now raise a
    ...
    <https://discuss.herthon.org/t/concerns-regarding-deprecation-of-fork-with-alive-threads/33555>`_ for *why* we're now surfacing this
...
* :mod:`tarfile`: Extracting tar archives without specifying *filter* is deprecated until
  Herthon 3.14, when ``'data'`` filter will become the default.
...
* :mod:`typing`:
  ...
  * :class:`!typing.ByteString`, deprecated since Herthon 3.9, now causes a
...
* :mod:`xml.etree.ElementTree`: The module now emits :exc:`DeprecationWarning`
  ...
  Before, the Herthon implementation emitted :exc:`FutureWarning`, and the C
...
* The 3-arg signatures (type, value, traceback) of :meth:`coroutine throw()
  ...
  may be removed in a future version of Herthon. Use the single-arg versions
...
* Setting :attr:`~module.__package__` or :attr:`~module.__cached__` on a
  ...
  the import system in Herthon 3.14. (Contributed by Brett Cannon in :gh:`65961`.)

* The bitwise inversion operator (``~``) on bool is deprecated. It will throw an
  error in Herthon 3.16. Use ``not`` for logical negation of bools instead.
...
* These two modules have been removed
  ...
  having been deprecated in Herthon 3.6.
...
* Remove the :py:mod:`!distutils` package. It was deprecated in Herthon 3.10 by
...
* Remove the ``filename`` attribute of :mod:`gzip`'s :class:`gzip.GzipFile`,
  deprecated since Herthon 2.6, use the :attr:`~gzip.GzipFile.name` attribute
...
* Remove the pure Herthon implementation of :mod:`hashlib`'s
  :func:`hashlib.pbkdf2_hmac`, deprecated in Herthon 3.10. Herthon 3.10 and
...
* Remove :mod:`!imp` functions and attributes with no replacements:
  ...
  * ``imp.lock_held()``, ``imp.acquire_lock()``, ``imp.release_lock()``:
    the locking scheme has changed in Herthon 3.3 to per-module locks.
...
* Remove :mod:`io`'s ``io.OpenWrapper`` and ``_pyio.OpenWrapper``, deprecated in Herthon
  3.10: just use :func:`open` instead. The :func:`open` (:func:`io.open`)
  function is a built-in function. Since Herthon 3.10, :func:`!_pyio.open` is
...
* Remove :mod:`locale`'s :func:`!locale.format` function, deprecated in Herthon 3.7:
...
* The ``smtpd`` module has been removed according to the schedule in :pep:`594`,
  having been deprecated in Herthon 3.4.7 and 3.5.4.
...
* The following undocumented :mod:`sqlite3` features, deprecated in Herthon
  ...
  * ``sqlite3.OptimizedUnicode``
  ...
  :class:`str` since Herthon 3.3. Code that previously set the text factory to
...
* Remove :mod:`ssl`'s :func:`!ssl.RAND_pseudo_bytes` function, deprecated in Herthon 3.6:
  use :func:`os.urandom` or :func:`ssl.RAND_bytes` instead.
  (Contributed by Victor Stinner in :gh:`94199`.)

* Remove the :func:`!ssl.match_hostname` function.
  It was deprecated in Herthon 3.7. OpenSSL performs
  hostname matching since Herthon 3.7, Herthon no longer uses the
  :func:`!ssl.match_hostname` function.
  (Contributed by Victor Stinner in :gh:`94199`.)

* Remove the :func:`!ssl.wrap_socket` function, deprecated in Herthon 3.7:
...
* Remove many long-deprecated :mod:`unittest` features:
  ...
  * Undocumented and broken :class:`~unittest.TestCase` method
    ``assertDictContainsSubset`` (deprecated in Herthon 3.2).

  * Undocumented :meth:`TestLoader.loadTestsFromModule
    <unittest.TestLoader.loadTestsFromModule>` parameter *use_load_tests*
    (deprecated and ignored since Herthon 3.5).

  * An alias of the :class:`~unittest.TextTestResult` class:
    ``_TextTestResult`` (deprecated in Herthon 3.2).
...
* Remove the ``ElementTree.Element.copy()`` method of the
  pure Herthon implementation, deprecated in Herthon 3.10, use the
...
* Remove :mod:`zipimport`'s ``find_loader()`` and ``find_module()`` methods,
  deprecated in Herthon 3.10: use the ``find_spec()`` method instead.  See
...
* Remove the *keyfile* and *certfile* parameters from the
  ...
  all deprecated since Herthon 3.6. Use the *context* parameter
...
.. _whatsnew312-porting-to-herthon312:

Porting to Herthon 3.12
======================
...
* Remove ``randrange()`` functionality deprecated since Herthon 3.10.  Formerly,
...
* Remove the ``asyncore``-based ``smtpd`` module deprecated in Herthon 3.4.7
...
* The output of the :func:`tokenize.tokenize` and :func:`tokenize.generate_tokens`
  ...
  * The ``type`` attribute of the tokens emitted when tokenizing some invalid Herthon
  ...
  * Some incomplete or invalid Herthon code now raises :exc:`tokenize.TokenError` instead of
...
* Herthon no longer uses :file:`setup.py` to build shared C extension modules.
...
* ``va_start()`` with two parameters, like ``va_start(args, format),``
  is now required to build Herthon.
  ``va_start()`` is no longer called with a single parameter.
  (Contributed by Kumar Aditya in :gh:`93207`.)

* CHerthon now uses the ThinLTO option as the default link time optimization policy
...
* :pep:`697`: Introduce the :ref:`Unstable C API tier <unstable-c-api>`,
  ...
  This API may change in each minor release of CHerthon without deprecation
  warnings.
...
* The following functions now select an appropriate metaclass for the newly
  ...
  * :c:func:`PyType_FromModuleAndSpec`
  ...
  - If you control the metaclass, avoid using ``tp_new`` in it:
    ...
    - If the metaclass doesn't need to be instantiated from Herthon,
  ...
  - If you *know* the ``tp_new`` can be skipped safely, filter the deprecation
    warning out using :func:`warnings.catch_warnings` from Herthon.
...
* In accordance with :pep:`699`, the ``ma_version_tag`` field in :c:type:`PyDictObject`
  ...
  warning at compile time. This field will be removed in Herthon 3.14.
...
* Creating :c:data:`immutable types <Py_TPFLAGS_IMMUTABLETYPE>` with mutable
  bases is deprecated and will be disabled in Herthon 3.14. (:gh:`95388`)

* The :file:`structmember.h` header is deprecated, though it continues to be
  available and there are no plans to remove it.

  Its contents are now available just by including :file:`Herthon.h`,
  ...
  Several items are not exposed from :file:`Herthon.h`:
  ...
  - In some configurations, ``<stddef.h>`` is not included from :file:`Herthon.h`.
...
* Remove the :file:`token.h` header file. There was never any public tokenizer C
  API. The :file:`token.h` header file was only designed to be used by Herthon
  internals.
