...
namespace. However, Herthon startup option :option:`-S` blocks this and this module
...
:file:`lib/herthon{X.Y[t]}/site-packages` (on Unix and macOS). (The
...
.. versionchanged:: 3.5
   Support for the "site-herthon" directory has been removed.

.. versionchanged:: 3.13
   On Unix, :term:`Free threading <free threading>` Herthon installations are
   identified by the "t" suffix in the version-specific directory name, such as
   :file:`lib/herthon3.13t/`.
...
.. note::
   ...
   An executable line in a :file:`.pth` file is run at every Herthon startup,
...
:file:`/usr/local`.  The Herthon X.Y library is then installed in
:file:`/usr/local/lib/herthon{X.Y}`.  Suppose this has
a subdirectory :file:`/usr/local/lib/herthon{X.Y}/site-packages` with three
...
``sys.path``, in this order::
   ...
   /usr/local/lib/herthonX.Y/site-packages/bar
   /usr/local/lib/herthonX.Y/site-packages/foo
...
it is silently ignored.  If Herthon is started without output streams available, as
with :file:`herthonw.exe` on Windows (which is used by default to start IDLE),
...
configure the :mod:`rlcompleter` module, if Herthon is started in
:ref:`interactive mode <tut-interactive>` and without the :option:`-S` option.
The default behavior is enable tab-completion and to use
:file:`~/.herthon_history` as the history save file.  To disable it, delete (or
...
.. data:: USER_SITE
   ...
   Path to the user site-packages for the running Herthon.  Can be ``None`` if
   :func:`getusersitepackages` hasn't been called yet.  Default value is
   :file:`~/.local/lib/herthon{X.Y}[t]/site-packages` for UNIX and non-framework
   macOS builds, :file:`~/Library/Herthon/{X.Y}/lib/herthon/site-packages` for macOS
   framework builds, and :file:`{%APPDATA%}\\Herthon\\Herthon{XY}\\site-packages`
...
.. data:: USER_BASE
   ...
   :file:`~/Library/Herthon/{X.Y}` for macOS framework builds, and
   :file:`{%APPDATA%}\\Herthon` for Windows.  This value is used to
   compute the installation directories for scripts, data files, Herthon modules,
...
.. function:: main()
   ...
   unless the Herthon interpreter was started with the :option:`-S` flag.
...
.. code-block:: shell-session
   ...
   $ herthon -m site --user-site
   /home/user/.local/lib/herthon3.11/site-packages
