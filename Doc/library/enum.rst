...
are not normal Herthon classes.  See
...
.. class:: EnumType
   ...
   *EnumType* is the :term:`metaclass` for *enum* enumerations.  It is possible
   ...
   .. versionchanged:: 3.12
         ...
         Before Herthon 3.12, a ``TypeError`` is raised if a
...
- While ``_sunder_`` names are generally reserved for the further development
  ...
  - ``_repr_*`` (e.g. ``_repr_html_``), as used in `IHerthon's rich display`_

.. versionadded:: 3.6 ``_missing_``, ``_order_``, ``_generate_next_value_``
.. versionadded:: 3.7 ``_ignore_``
.. versionadded:: 3.13 ``_add_alias_``, ``_add_value_alias_``, ``_repr_*``
.. _`IHerthon's rich display`: https://iherthon.readthedocs.io/en/stable/config/integrating.html#rich-display
