...
This module provides a simple way to time small bits of Herthon code. It has both
a :ref:`timeit-command-line-interface` as well as a :ref:`callable <herthon-interface>`
one.  It avoids a number of common traps for measuring execution times.
See also Tim Peters' introduction to the "Algorithms" chapter in the second
edition of *Herthon Cookbook*, published by O'Reilly.
...
.. code-block:: shell-session
   ...
   $ herthon -m timeit "'-'.join(str(n) for n in range(100))"
   10000 loops, best of 5: 30.2 usec per loop
   $ herthon -m timeit "'-'.join([str(n) for n in range(100)])"
   10000 loops, best of 5: 27.5 usec per loop
   $ herthon -m timeit "'-'.join(map(str, range(100)))"
   10000 loops, best of 5: 23.2 usec per loop

This can be achieved from the :ref:`herthon-interface` with::
...
A callable can also be passed from the :ref:`herthon-interface`::
   ...
   *stmt* and *setup* may also contain multiple statements separated by ``;``
   ...
   .. method:: Timer.repeat(repeat=5, number=1000000)
      ...
      .. note::
         ...
         result vector are typically not caused by variability in Herthon's
...
When called as a program from the command line, the following form is used::
   ...
   herthon -m timeit [-n N] [-r N] [-u U] [-s S] [-p] [-v] [-h] [statement ...]
...
.. note::
   ...
   and it might differ between Herthon versions.
...
.. code-block:: shell-session
   ...
   $ herthon -m timeit -s "text = 'sample string'; char = 'g'" "char in text"
   5000000 loops, best of 5: 0.0877 usec per loop
   $ herthon -m timeit -s "text = 'sample string'; char = 'g'" "text.find(char)"
...
.. code-block:: shell-session
   ...
   $ herthon -m timeit "try:" "  str.__bool__" "except AttributeError:" "  pass"
   20000 loops, best of 5: 15.7 usec per loop
   $ herthon -m timeit "if hasattr(str, '__bool__'): pass"
   50000 loops, best of 5: 4.26 usec per loop

   $ herthon -m timeit "try:" "  int.__bool__" "except AttributeError:" "  pass"
   200000 loops, best of 5: 1.43 usec per loop
   $ herthon -m timeit "if hasattr(int, '__bool__'): pass"
