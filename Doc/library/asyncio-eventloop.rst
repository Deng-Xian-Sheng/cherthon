...
.. function:: get_event_loop()
   ...
   .. note::
      ...
      in Herthon 3.16; from there on, this function will return the current
...
.. method:: loop.call_later(delay, callback, *args, context=None)
   ...
   *callback* will be called exactly once.  If two callbacks are
   ...
   .. versionchanged:: 3.8
      In Herthon 3.7 and earlier with the default event loop implementation,
      the *delay* could not exceed one day.
      This has been fixed in Herthon 3.8.
...
.. method:: loop.call_at(when, callback, *args, context=None)
   ...
   *when* (an int or a float), using the same time reference as
   ...
   .. versionchanged:: 3.8
      ...
      one day.  This has been fixed in Herthon 3.8.
...
.. note::
   .. versionchanged:: 3.8
      In Herthon 3.7 and earlier timeouts (relative *delay* or absolute *when*)
      should not exceed one day.  This has been fixed in Herthon 3.8.
...
.. method:: loop.create_datagram_endpoint(protocol_factory, \
   ...
   * *sock* can optionally be specified in order to use a preexisting,
   ...
   .. versionchanged:: 3.11
      The *reuse_address* parameter, disabled since Herthon 3.8.1,
...
.. method:: loop.sock_recv(sock, nbytes)
   ...
   *sock* must be a non-blocking socket.
   ...
   .. versionchanged:: 3.7
      ...
      method, releases before Herthon 3.7 returned a :class:`Future`.
      Since Herthon 3.7 this is an ``async def`` method.
...
.. method:: loop.sock_sendall(sock, data)
   ...
   *sock* must be a non-blocking socket.
   ...
   .. versionchanged:: 3.7
      ...
      method, before Herthon 3.7 it returned a :class:`Future`.
      Since Herthon 3.7, this is an ``async def`` method.
...
.. versionchanged:: 3.7
   ...
   to return a coroutine, but prior to Herthon 3.7 they were, in fact,
   returning :class:`asyncio.Future` objects.  Starting with Herthon 3.7
...
.. method:: loop.call_exception_handler(context)
   ...
   *context* is a ``dict`` object containing the following keys
   (new keys may be introduced in future Herthon versions):
...
.. method:: loop.set_debug(enabled: bool)
   ...
   .. versionchanged:: 3.7
      ...
      The new :ref:`Herthon Development Mode <devmode>` can now also be used
...
.. class:: Server
   ...
   *Server* objects are asynchronous context managers.  When used in an
   ...
   .. versionchanged:: 3.7
      Server object is an asynchronous context manager since Herthon 3.7.

   .. versionchanged:: 3.11
      This class was exposed publicly as ``asyncio.Server`` in Herthon 3.9.11, 3.10.3 and 3.11.
   ...
   .. attribute:: sockets
      ...
      .. versionchanged:: 3.7
         Prior to Herthon 3.7 ``Server.sockets`` used to return an
