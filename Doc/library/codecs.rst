...
This module defines base classes for standard Herthon codecs (encoders and
decoders) and provides access to the internal Herthon codec registry, which
...
.. function:: lookup(encoding, /)
   ...
   Looks up the codec info in the Herthon codec registry and returns a
...
Each codec has to define four interfaces to make it usable as codec in Herthon:
...
The following error handlers can be used with all Herthon
...
define in order to be compatible with the Herthon codec registry.
...
compatible with the Herthon codec registry.
...
detected, Microsoft invented a variant of UTF-8 (that Herthon calls
...
Herthon comes with a number of codecs built-in, either implemented as C functions
...
.. impl-detail::
   ...
   recognized by CHerthon for a limited set of (case insensitive)
...
Herthon Specific Encodings
-------------------------

A number of predefined codecs are specific to Herthon, so their codec names have
no meaning outside Herthon. These are listed in the tables below based on the
...
Herthon supports this conversion in several ways:  the ``idna`` codec performs
