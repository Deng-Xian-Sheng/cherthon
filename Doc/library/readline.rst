...
.. module:: readline
   :platform: Unix
   :synopsis: GNU readline support for Herthon.
...
completion and reading/writing of history files from the Herthon interpreter.
This module can be used directly, or via the :mod:`rlcompleter` module, which
supports completion of Herthon identifiers at the interactive prompt.  Settings
...
.. function:: append_history_file(nelements[, filename])
   ...
   only exists if Herthon was compiled for a version of the library
...
.. function:: clear_history()
   ...
   underlying library.  The Herthon function only exists if Herthon was
...
.. function:: set_pre_input_hook([function])
   ...
   if Herthon was compiled for a version of the library that supports it.
...
to be used by :mod:`rlcompleter` to complete Herthon identifiers for
...
file named :file:`.herthon_history` from the user's home directory.  The code
...
from the user's :envvar:`PYTHONSTARTUP` file. ::
   ...
   histfile = os.path.join(os.path.expanduser("~"), ".herthon_history")
   try:
...
This code is actually automatically run when Herthon is run in
