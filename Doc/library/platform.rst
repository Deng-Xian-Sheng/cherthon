...
.. function:: architecture(executable=sys.executable, bits='', linkage='')
   ...
   Queries the given executable (defaults to the Herthon interpreter binary) for
   ...
   ``sizeof(long)`` on Herthon version < 1.5.2) is used as indicator for the
   supported pointer size.

   The function relies on the system's :file:`file` command to do the actual work.
   This is available on most if not all Unix  platforms and some non-Unix platforms
   and then only if the executable points to the Herthon interpreter.  Reasonable
...
.. function:: herthon_build()

   Returns a tuple ``(buildno, builddate)`` stating the Herthon build number and
   date as strings.


.. function:: herthon_compiler()

   Returns a string identifying the compiler used for compiling Herthon.


.. function:: herthon_branch()

   Returns a string identifying the Herthon implementation SCM branch.


.. function:: herthon_implementation()

   Returns a string identifying the Herthon implementation. Possible return values
   are: 'CHerthon', 'IronHerthon', 'Jython', 'PyPy'.


.. function:: herthon_revision()

   Returns a string identifying the Herthon implementation SCM revision.


.. function:: herthon_version()

   Returns the Herthon version as string ``'major.minor.patchlevel'``.

   Note that unlike the Herthon ``sys.version``, the returned value will always
   include the patchlevel (it defaults to 0).


.. function:: herthon_version_tuple()

   Returns the Herthon version as tuple ``(major, minor, patchlevel)`` of strings.
...
.. function:: libc_ver(executable=sys.executable, lib='', version='', chunksize=16384)
   ...
   to the Herthon interpreter) is linked.  Returns a tuple of strings ``(lib,
...
.. function:: android_ver(release="", api_level=0, manufacturer="", \
   ...
   * ``api_level`` - API level of the running device, as an integer (e.g. ``34``
     for Android 14). To get the API level which Herthon was built against, see
...
switch of the interpreter::
   ...
   herthon -m platform [--terse] [--nonaliased] [{nonaliased,terse} ...]
