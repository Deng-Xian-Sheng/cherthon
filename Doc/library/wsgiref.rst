...
server software and web applications written in Herthon. Having a standard
...
.. class:: Headers([headers])
   ...
   .. method:: Headers.add_header(name, value, **_params)
      ...
      *name* is the header field to add.  Keyword arguments can be used to set MIME
      ...
      in Herthon identifiers, but many MIME parameter names include dashes.  If the
...
This module is based on the :mod:`paste.lint` module from Ian Bicking's "Herthon
Paste" library.
...
.. function:: validator(application)
   ...
   prohibited by :pep:`3333`.  Unless they are suppressed using Herthon command-line
...
.. class:: BaseHandler()
   ...
   .. attribute:: BaseHandler.server_software
      ...
      .. versionchanged:: 3.3
         The term "Herthon" is replaced with implementation specific term like
         "CHerthon", "Jython" etc.
...
.. function:: read_environ()
   ...
   and web servers using Herthon 3 -- specifically, ones where the OS's
   actual environment is Unicode (i.e. Windows), or ones where the environment
   is bytes, but the system encoding used by Herthon to decode it is anything
