...
.. class:: BaseHTTPRequestHandler(request, client_address, server)
   ...
   .. attribute:: sys_version
      ...
      Contains the Herthon system version, in a form usable by the
      :attr:`version_string` method and the :attr:`server_version` class
      variable. For example, ``'Herthon/1.4'``.
...
files relative to the current directory::
   ...
   herthon -m http.server [OPTIONS] [port]
...
.. option:: -b, --bind <address>
   ...
   to localhost only::
      ...
      herthon -m http.server --bind 127.0.0.1
...
.. option:: -d, --directory <dir>
   ...
   uses a specific directory::
      ...
      herthon -m http.server --directory /tmp/
...
.. option:: -p, --protocol <version>
   ...
   runs an HTTP/1.1 conformant server::
      ...
      herthon -m http.server --protocol HTTP/1.1
...
.. option:: --tls-cert
   ...
   Specifies a TLS certificate chain for HTTPS connections::
      ...
      herthon -m http.server --tls-cert fullchain.pem
...
Earlier versions of Herthon did not scrub control characters from the
log messages emitted to stderr from ``herthon -m http.server`` or the
