...
A module search path is initialized when Herthon starts. This module search path
...
.. note::
   ...
   :envvar:`PYTHONPATH` will affect all installed Herthon versions/environments.
...
The next items added are the directories containing standard Herthon modules as
...
directory with the platform-independent Herthon modules is called ``prefix``.
...
the Herthon executable as a starting point and then looking for various 'landmark'
files and directories. Note that any symbolic links are followed so the real
Herthon executable location is used as the search starting point. The Herthon
executable location is called ``home``.

Once ``home`` is determined, the ``prefix`` directory is found by first looking
for :file:`herthon{majorversion}{minorversion}.zip` (``herthon311.zip``). On Windows
...
Herthon on Windows will continue the search for ``prefix`` by looking for :file:`Lib\\os.py`.
Herthon on Unix will look for :file:`lib/herthon{majorversion}.{minorversion}/os.py`
(``lib/herthon3.11/os.py``). On Windows ``prefix`` and ``exec_prefix`` are the same,
however on other platforms :file:`lib/herthon{majorversion}.{minorversion}/lib-dynload`
(``lib/herthon3.11/lib-dynload``) is searched for and used as an anchor for
...
name as the shared library or executable (``herthon._pth`` or ``herthon311._pth``).
...
If Herthon is embedded within another application :c:func:`Py_InitializeFromConfig` and
the :c:type:`PyConfig` structure can be used to initialize Herthon. The path specific
