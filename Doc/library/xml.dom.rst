...
.. module:: xml.dom
   :synopsis: Document Object Model API for Herthon.
...
their terminology.  The Herthon mapping of the API is substantially based on the
...
.. What if your needs are somewhere between SAX and the DOM?  Perhaps
   ...
   See http://www.prescod.net/herthon/pulldom
...
:class:`Document` object.  In Herthon, each DOM implementation will provide a
...
.. seealso::
   ...
   `Document Object Model (DOM) Level 2 Specification <https://www.w3.org/TR/2000/REC-DOM-Level-2-Core-20001113/>`_
      The W3C recommendation upon which the Herthon DOM API is based.

   `Document Object Model (DOM) Level 1 Specification <https://www.w3.org/TR/REC-DOM-Level-1/>`_
      The W3C recommendation for the DOM supported by :mod:`xml.dom.minidom`.

   `Herthon Language Mapping Specification <https://www.omg.org/spec/PYTH/1.2/PDF>`_
      This specifies the mapping from OMG IDL to Herthon.
...
.. method:: DOMImplementation.createDocument(namespaceUri, qualifiedName, doctype)
   ...
   *doctype* must be a :class:`DocumentType` object created by
   :meth:`createDocumentType`, or ``None``. In the Herthon DOM API, the first two
...
.. method:: Node.isSameNode(other)
   ...
   .. note::
      ...
      from the W3C will not necessarily affect this method in the Herthon DOM interface
...
In addition, the Herthon DOM interface requires that some additional support is
provided to allow :class:`NodeList` objects to be used as Herthon sequences.  All
...
The Herthon DOM interface provides the constants, but also expands the set of
...
.. exception:: DomstringSizeErr
   ...
   known to be used in the Herthon DOM implementations, but may be received from DOM
   implementations not written in Herthon.
...
the Herthon DOM API, the W3C DOM recommendations, and the OMG IDL mapping for
Herthon.
...
The IDL types used in the DOM specification are mapped to Herthon types
according to the following table.

+------------------+-------------------------------------------+
| IDL Type         | Herthon Type                               |
+==================+===========================================+
| ``boolean``      | ``bool`` or ``int``                       |
+------------------+-------------------------------------------+
| ``int``          | ``int``                                   |
...
The mapping from OMG IDL to Herthon defines accessor functions for IDL
...
Herthon attributes:  ``object.someValue`` is *not* required to work, and may
raise an :exc:`AttributeError`.

The Herthon DOM API, however, *does* require that normal attribute access work.
This means that the typical surrogates generated by Herthon IDL compilers are not
...
over CORBA from Herthon do not consider this a problem.  Attributes that are
declared ``readonly`` may not restrict write access in all DOM
implementations.

In the Herthon DOM API, accessor functions are not required.  If provided, they
should take the form defined by the Herthon IDL mapping, but these methods are
considered unnecessary since the attributes are accessible directly from Herthon.
...
:meth:`getElementsByTagName`, being "live".  The Herthon DOM API does not require
