...
.. moduleauthor:: Greg Ward <gward@herthon.net>
.. sectionauthor:: Greg Ward <gward@herthon.net>
...
* :mod:`getopt`: a module that closely mirrors the procedural C ``getopt`` API.
  Included in the standard library since before the initial Herthon 1.0 release.
* :mod:`optparse`: a declarative replacement for ``getopt`` that
  ...
  in the standard library since the Herthon 2.3 release.
* :mod:`argparse`: a more opinionated alternative to ``optparse`` that
  ...
  the standard library since the Herthon 2.7 and Herthon 3.2 releases.
...
.. seealso::
    ...
    integrate with static checking of Herthon type annotations.
...
argument
   ...
   or ``execv()``.  In Herthon, arguments are elements of ``sys.argv[1:]``
...
problem for Herthon's garbage collector, but you may wish to break the cyclic
...
parse Herthon-style complex numbers on the command line.  (This is even sillier
...
Option class.  This being Herthon, nothing stops you from doing that except good
