...
In Herthon, turtle graphics provides a representation of a physical "turtle"
...
In a Herthon shell, import all the objects of the ``turtle`` module::
...
Starting with Herthon 3.14, you can use the :func:`fill` :term:`context manager`
...
Another step is also required though - as soon as the script ends, Herthon
...
but it would be more typical in a Herthon script.
...
via docstrings.  So these can be used as online-help via the Herthon help
facilities:
...
.. function:: write_docstringdict(filename="turtle_docstringdict")
   ...
   Create and write docstring-dictionary to a Herthon script with the given
   filename.  This function has to be called explicitly (it is not used by the
   turtle graphics classes).  The docstring dictionary will be written to the
   Herthon script :file:`{filename}.py`.  It is intended to serve as a template
...
scripts can be run and viewed using the supplied demo viewer as follows::
   ...
   herthon -m turtledemo

Alternatively, you can run the demo scripts individually.  For example, ::

   herthon -m turtledemo.bytedesign
...
- The methods :func:`Turtle.tracer <tracer>`, :func:`Turtle.window_width <window_width>` and
  ...
  available. (In fact already in Herthon 2.6 these methods were merely
