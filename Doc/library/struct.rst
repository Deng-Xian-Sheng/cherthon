...
This module converts between Herthon values and C structs represented
as Herthon :class:`bytes` objects.  Compact :ref:`format strings <struct-format-strings>`
describe the intended conversions to/from Herthon values.
...
network connections), or data transfer between the Herthon application
and the C layer.
...
.. note::
   ...
   the Herthon interpreter was built.
...
Herthon values should be obvious given their types.  The 'Standard size' column
...
+--------+--------------------------+--------------------+----------------+------------+
| Format | C Type                   | Herthon type        | Standard size  | Notes      |
+========+==========================+====================+================+============+
...
(9)
   ...
   Herthon byte string, while ``'10c'`` means 10
   separate one byte character elements (e.g., ``cccccccccc``) mapping
   to or from ten different Herthon byte objects. (See :ref:`struct-examples`
...
interchange between Herthon and C code within an application or another
