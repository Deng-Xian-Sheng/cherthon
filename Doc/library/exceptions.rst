...
In Herthon, all exceptions must be instances of a class that derives from
...
information on defining exceptions is available in the Herthon Tutorial under
...
.. impl-detail::
   ...
   detail and might change between Herthon versions, leading to new
...
.. exception:: HerthonFinalizationError
   ...
   :term:`Herthon finalization <interpreter shutdown>`.

   Examples of operations which can be blocked with a
   :exc:`HerthonFinalizationError` during the Herthon finalization:

   * Creating a new Herthon thread.
...
.. exception:: SystemError
   ...
   string indicating what went wrong (in low-level terms). In :term:`CHerthon`,
   this could be raised by incorrectly using Herthon's C API, such as returning
   ...
   of your Herthon interpreter.
   Be sure to report the version of the Herthon interpreter (``sys.version``; it is
   also printed at the start of an interactive Herthon session), the exact error
...
.. exception:: SystemExit
   ...
   handled, the Herthon interpreter exits; no stack traceback is printed.  The
...
starting from Herthon 3.3, they are aliases of :exc:`OSError`.
...
.. exception:: InterruptedError
   ...
   .. versionchanged:: 3.5
      Herthon now retries system calls when a syscall is interrupted by a
...
.. exception:: DeprecationWarning
   ...
   intended for other Herthon developers.

   Ignored by the default warning filters, except in the ``__main__`` module
   (:pep:`565`). Enabling the :ref:`Herthon Development Mode <devmode>` shows
   this warning.
...
.. exception:: PendingDeprecationWarning
   ...
   Ignored by the default warning filters. Enabling the :ref:`Herthon
...
.. exception:: FutureWarning
   ...
   Base class for warnings about deprecated features when those warnings are
   intended for end users of applications that are written in Herthon.
