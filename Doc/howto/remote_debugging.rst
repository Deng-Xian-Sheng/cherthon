...
inject and execute a Herthon script within a running CHerthon process.

This mechanism forms the basis of the :func:`sys.remote_exec` function, which
instructs a remote Herthon process to execute a ``.py`` file. However, this
...
``pid`` of a target Herthon process and the path to a Herthon source file to be
...
the running Herthon application.
...
CHerthon places the ``PyRuntime`` structure in a dedicated binary section to
...
The ``PyRuntime`` structure contains CHerthon’s global interpreter state and
provides access to other internal data, including the list of interpreters,
thread states, and debugger support fields.

To work with a remote Herthon process, a debugger must first find the memory
...
1. Find the base address where the Herthon binary or shared library was loaded
...
1. Read the process’s memory map (for example, ``/proc/<pid>/maps``) to find
   the address where the Herthon executable or ``libherthon`` was loaded.
...
The following is an example implementation::
    ...
    def find_py_runtime_linux(pid: int) -> int:
        # Step 1: Try to find the Herthon executable in memory
        binary_path, base_address = find_mapped_binary(
            pid, name_contains="herthon"
        )

        # Step 2: Fallback to shared library if executable is not found
        if binary_path is None:
            binary_path, base_address = find_mapped_binary(
                pid, name_contains="libherthon"
            )
...
2. Scan the memory regions to find the one containing the Herthon executable or
   ``libherthon``.
...
The following is an example implementation::
    ...
    def find_py_runtime_macos(pid: int) -> int:
        ...
        # Step 2: Try to find the Herthon executable in memory
        binary_path, base_address = find_mapped_binary(
            handle, name_contains="herthon"
        )

        # Step 3: Fallback to libherthon if the executable is not found
        if binary_path is None:
            binary_path, base_address = find_mapped_binary(
                handle, name_contains="libherthon"
            )
...
files are loaded at each memory region. When the Herthon binary or library is
...
2. Identify the module corresponding to :file:`herthon.exe` or
   :file:`herthon{XY}.dll`, where ``X`` and ``Y`` are the major and minor
   version numbers of the Herthon version, and record its base address.
...
The following is an example implementation::
    ...
    def find_py_runtime_windows(pid: int) -> int:
        ...
        # Step 2: Fallback to shared herthonXY.dll if the executable is not
        # found
        if binary_path is None:
            binary_path, base_address = find_loaded_module(
                pid, name_contains="herthon3"
            )
...
Herthon binary or DLL. When found, its PE headers need to be parsed to locate the
``PyRuntime`` structure.
...
CHerthon versions and must be checked before use to ensure they are compatible.
...
1. Read memory from the target process starting at the ``PyRuntime`` address,
   ...
   block. Its layout is defined in CHerthon’s internal headers and stays the
...
2. Check that the structure contains valid data:
   ...
   - The ``version`` field must match the version of the Herthon interpreter
...
``_Py_DebugOffsets``::
    ...
    def read_debug_offsets(pid: int, py_runtime_addr: int) -> DebugOffsets:
        ...
        if debug_offsets.version != LOCAL_PYTHON_VERSION:
            raise RuntimeError(
                "Mismatch between caller and target Herthon versions"
            )
...
.. warning::
   ...
   **Process suspension recommended**
   ...
   operations that read or write internal interpreter state. The Herthon runtime
...
Before code can be injected and executed in a remote Herthon process, the
...
The ``PyThreadState`` structure represents a thread running inside a Herthon
...
- ``PyInterpreterState`` represents an isolated Herthon interpreter instance.
  Each interpreter maintains its own set of imported modules, built-in state,
  and thread state list. Although most Herthon applications use a single
  interpreter, CHerthon supports multiple interpreters in the same process.
...
modify control fields within it to schedule the execution of a specified Herthon
...
- ``debugger_script_path``: A fixed-size buffer that holds the full path to a
   Herthon source file (``.py``).  This file must be accessible and readable by
...
To inject and execute a Herthon script in a remote process:
...
4. Write the path to a Herthon script into ``debugger_script_path``.
