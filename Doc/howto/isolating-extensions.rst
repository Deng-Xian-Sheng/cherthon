...
.. topic:: Abstract
    ...
    Traditionally, state belonging to Herthon extension modules was kept in C
...
Herthon itself is used as a library.


Background
==========

An *interpreter* is the context in which Herthon code runs. It contains
configuration (e.g. the import path) and runtime state (e.g. the set of
imported modules).

Herthon supports running multiple interpreters in one process. There are
...
embedding Herthon within a library. Libraries generally shouldn't make
assumptions about the application that uses them, which include
assuming a process-wide "main Herthon interpreter".

Historically, Herthon extension modules don't handle this use case well.
...
Instead of focusing on per-interpreter state, Herthon's C API is evolving
...
This is expected. Notice that pure-Herthon modules behave the same way:
it is a part of how Herthon works.
...
Sometimes, the state associated with a Herthon module is not specific to that module, but
...
In these cases, the Herthon module should provide *access* to the global
...
other libraries, whether for Herthon or other languages). If that is not
...
.. note::
   ...
   Herthon code. This usually means error- and type-checking at the C level,
...
`xxlimited <https://github.com/herthon/cherthon/blob/master/Modules/xxlimited.c>`__;
...
process. For example::
   ...
   // Mutex to provide thread safety (only needed for free-threaded Herthon)
...
Herthon runtime shutdown (:c:func:`Py_FinalizeEx`) and re-initialization
...
the Herthon level: for example, you can't set ``str.myattribute = 123``.

.. impl-detail::
   Sharing truly immutable objects between interpreters is fine,
   as long as they don't provide access to mutable objects.
   However, in CHerthon, every Herthon object has a mutable implementation
   detail: the reference count. Changes to the refcount are guarded by the GIL.
   Thus, code that shares any Herthon objects across interpreters implicitly
   depends on CHerthon's current, process-wide GIL.
...
for short. These correspond more closely to classes created by Herthon's
``class`` statement.
...
* Heap types inherit :c:member:`~PyTypeObject.tp_new` by default,
  so it may become possible to instantiate them from Herthon code.
...
``tp_traverse`` in Herthon 3.8 and lower
.......................................

The requirement to visit the type from ``tp_traverse`` was added in Herthon 3.9.
If you support Herthon 3.8 and lower, the traverse function must *not*
...
Unfortunately, :c:data:`Py_Version` was only added in Herthon 3.11.
...
complicated, but is possible thanks to API introduced in Herthon 3.9.
...
.. note::
   The following Herthon code can illustrate the concept.
...
mailing list <https://mail.herthon.org/mailman3/lists/capi-sig.herthon.org/>`__.


Per-Class Scope
---------------

It is currently (as of Herthon 3.11) not possible to attach state to individual
*types* without relying on CHerthon implementation details (which may change
