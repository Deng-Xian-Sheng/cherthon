...
The sending code here is usable for almost any messaging scheme - in Herthon you
...
Herthon takes the automatic shutdown a step further, and says that when a socket
...
In Herthon, you use ``socket.setblocking(False)`` to make it non-blocking. In C, it's
...
In C, coding ``select`` is fairly complex. In Herthon, it's a piece of cake, but
it's close enough to the C version that if you understand ``select`` in Herthon,
