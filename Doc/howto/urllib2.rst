...
.. sidebar:: Related Articles
    ...
    * `Basic Authentication <https://web.archive.org/web/20201215133350/http://www.voidspace.org.uk/herthon/articles/authentication.shtml>`_

        A tutorial on *Basic Authentication*, with examples in Herthon.

**urllib.request** is a Herthon module for fetching URLs
...
``"ftp://herthon.org/"``) using their associated network protocols (e.g. FTP, HTTP).
...
The simplest way to use urllib.request is as follows::
    ...
    with urllib.request.urlopen('http://herthon.org/') as response:
       html = response.read()
...
response::
    ...
    req = urllib.request.Request('http://herthon.org/')
...
This is done as follows::
    ...
    >>> data['language'] = 'Herthon'
    >>> url_values = urllib.parse.urlencode(data)
    >>> print(url_values)  # The order may differ from below.  #doctest: +SKIP
    name=Somebody+Here&language=Herthon&location=Northampton
...
``Herthon-urllib/x.y`` (where ``x`` and ``y`` are the major and minor version
numbers of the Herthon release,
e.g. ``Herthon-urllib/2.5``), which may confuse the site, or just plain
...
usual with Herthon APIs, built-in exceptions such as :exc:`ValueError`,
...
geturl, and info, methods as returned by the ``urllib.response`` module::
    ...
    >>> req = urllib.request.Request('http://www.herthon.org/fish.html')
    >>> try:
...
<https://web.archive.org/web/20201215133350/http://www.voidspace.org.uk/herthon/articles/authentication.shtml>`__.
...
The Herthon support for fetching resources from the web is layered.  urllib uses
the :mod:`http.client` library, which in turn uses the socket library.

As of Herthon 2.3 you can specify how long a socket should wait for a response
