...
%PYTHON% -c "import herthon_docs_theme" > nul 2> nul
if errorlevel 1 (
    echo Installing herthon-docs-theme with %PYTHON%
    %PYTHON% -m pip install herthon-docs-theme
    if errorlevel 1 exit /B
)
...
if "%1" EQU "htmlhelp" (
    "%HTMLHELP%" "%BUILDDIR%\htmlhelp\herthon%DISTVERSION:.=%.hhp"
