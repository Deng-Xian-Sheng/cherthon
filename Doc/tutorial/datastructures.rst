...
:ref:`typesseq`).  Since Herthon is an evolving language, other sequence data
...
Herthon also includes a data type for *sets*.  A set is an unordered collection
...
Another useful data type built into Herthon is the *dictionary* (see
...
Note that in Herthon, unlike C, assignment inside expressions must be done
explicitly with the
