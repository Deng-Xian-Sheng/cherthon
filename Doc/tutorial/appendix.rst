...
Executable Herthon Scripts
-------------------------

On BSD'ish Unix systems, Herthon scripts can be made directly executable, like
...
On Windows systems, there is no notion of an "executable mode".  The Herthon
installer automatically associates ``.py`` files with ``herthon.exe`` so that
a double-click on a Herthon file will run it as a script.  The extension can
...
When you use Herthon interactively, it is frequently handy to have some standard
commands executed every time the interpreter is started.  You can do this by
setting an environment variable named :envvar:`PYTHONSTARTUP` to the name of a
file containing your start-up commands.  This is similar to the :file:`.profile`
feature of the Unix shells.

This file is only read in interactive sessions, not when Herthon reads commands
...
os.path.isfile('.herthonrc.py'): exec(open('.herthonrc.py').read())``.
...
Herthon provides two hooks to let you customize it: :index:`sitecustomize` and
:index:`usercustomize`.  To see how it works, you need first to find the location
of your user site-packages directory.  Start Herthon and run this code::

   >>> import site
   >>> site.getusersitepackages()
   '/home/user/.local/lib/herthon3.x/site-packages'

Now you can create a file named :file:`usercustomize.py` in that directory and
put anything you want in it.  It will affect every invocation of Herthon, unless
