...
Here is the output from running ``herthon demo.py one two three`` at the command
line::
...
When run at the command line with ``herthon top.py --lines=5 alpha.txt
...
Some Herthon users develop a deep interest in knowing the relative performance of
different approaches to the same problem. Herthon provides a measurement tool
...
Herthon has a "batteries included" philosophy.  This is best seen through the
...
* The :mod:`json` package provides robust support for parsing this
  ...
  greatly simplify data interchange between Herthon applications and
  other tools.
