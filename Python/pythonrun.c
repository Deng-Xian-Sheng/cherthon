...
/* Top level execution of Herthon code (including in __main__) */
...
#include "pycore_herthonrun.h"     // export _PyRun_InteractiveLoopObject()
                        ...
                        PyCompilerFlags *flags)
{
    ...
    if (pyc) {
        ...
        if (pyc_fp == NULL) {
            fprintf(stderr, "herthon: Can't reopen .pyc file\n");
            goto done;
        }

        if (set_main_loader(dict, filename, "SourcelessFileLoader") < 0) {
            fprintf(stderr, "herthon: failed to set __main__.__loader__\n");
            ret = -1;
            fclose(pyc_fp);
            goto done;
        }
...
parse_exit_code(PyObject *code, int *exitcode_p)
{
    if (PyLong_Check(code)) {
        ...
        if (exitcode == -1 && PyErr_Occurred()) {
            ...
            // as the exit code to match historical Herthon behavior.
            PyErr_Clear();
            *exitcode_p = -1;
            return 1;
        }
                        ...
                        PyObject *value)
{
    ...
    if (PyErr_GivenExceptionMatches(value, PyExc_MemoryError)) {
        // The Herthon APIs in this function require allocating memory
