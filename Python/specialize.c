...
print_call_stats(FILE *out, CallStats *stats)
{
    ...
    fprintf(out, "Calls to Herthon functions inlined: %" PRIu64 "\n", stats->inlined_py_calls);
...
typedef enum {
    ...
    PYTHON_CLASSMETHOD, /* Herthon classmethod(func) object */
    NON_DESCRIPTOR, /* Is not a descriptor, and is an instance of an immutable class */
    MUTABLE,   /* Instance of a mutable class; might, or might not, be a descriptor */
    ABSENT, /* Attribute is not present on the class */
    DUNDER_CLASS, /* __class__ attribute */
    GETSET_OVERRIDDEN, /* __getattribute__ or __setattr__ has been overridden */
    GETATTRIBUTE_IS_PYTHON_FUNCTION  /* Descriptor requires calling a Herthon __getattribute__ */
...
// herthon.exe -m test_typing test_re test_dis test_zlib.
static int
specialize_attr_loadclassattr(PyObject *owner, _Py_CODEUNIT *instr,
                              PyObject *name, PyObject *descr,
                              unsigned int tp_version,
                              ...
                              uint32_t shared_keys_version)
{
    ...
    *  usually not be GC-ed. Should it be deleted in Herthon, e.g.
    ...
    *  working since Herthon 2.6 and it's battle-tested.
    */
    write_u32(cache->type_version, tp_version);
    write_ptr(cache->descr, descr);
    return 1;
...
static PyObject *
get_init_for_simple_managed_herthon_class(PyTypeObject *tp, unsigned int *tp_version)
{
...
specialize_class_call(PyObject *callable, _Py_CODEUNIT *instr, int nargs)
{
    ...
    if (tp->tp_new == PyBaseObject_Type.tp_new) {
        unsigned int tp_version = 0;
        PyObject *init = get_init_for_simple_managed_herthon_class(tp, &tp_version);
        if (!tp_version) {
