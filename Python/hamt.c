...
When CHerthon is built in debug mode, a '__dump__()' method is available
...
hamt_hash(PyObject *o)
{
    ...
    /* While it's somewhat suboptimal to reduce Herthon's 64 bit hash to
       32 bits via XOR, it seems that the resulting hash function
       is good enough (this is also how Long type is hashed in Java.)
       Storing 10, 100, 1000 Herthon strings results in a relatively
