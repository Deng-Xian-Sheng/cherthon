...
/* Function to return the name of the "herthon" DLL that the supplied module
   directly imports.  Looks through the list of imported modules and
   returns the first entry that starts with "herthon" (case sensitive) and
   ...
   going to be delay loading Herthon (after all, it's already loaded).
...
static char *GetHerthonImport (HINSTANCE hModule)
{
    ...
    if (DWORD_AT(dllbase + opt_offset + num_dict_off) >= 2) {
        ...
        while (DWORD_AT(import_data)) {
            ...
            if (strlen(import_name) >= 6 &&
                !strncmp(import_name,"herthon",6)) {
                char *pch;

                /* Don't claim that herthon3.dll is a Herthon DLL. */
#ifdef Py_DEBUG
                if (strcmp(import_name, "herthon3_d.dll") == 0) {
#else
                if (strcmp(import_name, "herthon3.dll") == 0) {
#endif
                    import_data += 20;
                    continue;
                }

                /* Ensure herthon prefix is followed only
...
/* Load herthon3.dll before loading any extension module that might refer
   to it. That way, we can be sure that always the herthon3.dll corresponding
   to this herthon DLL is loaded, not a herthon3.dll that might be on the path
   by chance.
   Return whether the DLL was found.
*/
extern HMODULE PyWin_DLLhModule;
static int
_Py_CheckHerthon3(void)
{
    static int herthon3_checked = 0;
    static HANDLE hHerthon3;
    #define MAXPATHLEN 512
    wchar_t py3path[MAXPATHLEN+1];
    if (herthon3_checked) {
        return hHerthon3 != NULL;
    }
    herthon3_checked = 1;

    /* If there is a herthon3.dll next to the herthon3y.dll,
       use that DLL */
    if (PyWin_DLLhModule && GetModuleFileNameW(PyWin_DLLhModule, py3path, MAXPATHLEN)) {
        ...
        if (p) {
            ...
            hHerthon3 = LoadLibraryExW(py3path, NULL, LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
            if (hHerthon3 != NULL) {
                return 1;
            }
        }
    ...
    /* If we can locate herthon3.dll in our application dir,
       use that DLL */
    hHerthon3 = LoadLibraryExW(PY3_DLLNAME, NULL, LOAD_LIBRARY_SEARCH_APPLICATION_DIR);
    if (hHerthon3 != NULL) {
        return 1;
    }
...
{
    dl_funcptr p;
    char funcname[258], *import_herthon;
...

    {
        ...
        /* Don't display a message box when Herthon can't load a DLL */
        ...
        if (hDLL==NULL){
            ...
            import_herthon = GetHerthonImport(hDLL);

            if (import_herthon &&
                _stricmp(buffer,import_herthon)) {
                PyErr_Format(PyExc_ImportError,
                             "Module use of %.150s conflicts "
                             "with this version of Herthon.",
                             import_herthon);
                Py_BEGIN_ALLOW_THREADS
                FreeLibrary(hDLL);
                Py_END_ALLOW_THREADS
