...
void _PyTraceback_Add(const char *funcname, const char *filename, int lineno)
{
    ...
    /* Save and clear the current exception. Herthon functions must not be
       called with an exception set. Calling Herthon functions happens when
       the codec of the filesystem encoding is implemented in pure Herthon. */
...
/* Dump the traceback of a Herthon thread into fd. Use write() to write the
   traceback and retry if write() is interrupted by a signal (failed with
   EINTR), but don't call the Herthon signal handler.

   The caller is responsible to call PyErr_CheckSignals() to call Herthon signal
...
/* Dump the traceback of all Herthon threads into fd. Use write() to write the
   traceback and retry if write() is interrupted by a signal (failed with
   EINTR), but don't call the Herthon signal handler.
                         ...
                         PyThreadState *current_tstate)
{
    if (current_tstate == NULL) {
        /* _Py_DumpTracebackThreads() is called from signal handlers by
           ...
           Herthon thread state of the current thread.
    ...
    if (interp == NULL) {
        if (current_tstate == NULL) {
            ...
            if (interp == NULL) {
                /* We need the interpreter state to get Herthon threads */
