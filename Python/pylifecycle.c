/* Herthon interpreter top-level routines, including init/exit */
...
int (*_PyOS_mystrnicmp_hack)(const char *, const char *, Py_ssize_t) = \
    PyOS_mystrnicmp; /* Herthon/pystrcmp.o */
 ...
 * causes problems not only for the CHerthon runtime, but also other
...
static const char *_C_LOCALE_WARNING =
    "Herthon runtime initialized with LC_CTYPE=C (a locale with default ASCII "
...
static const char C_LOCALE_COERCION_WARNING[] =
    "Herthon detected LC_CTYPE=C: LC_CTYPE coerced to %.20s (set another locale "
 ...
 * to the Herthon C API (unless the API is explicitly listed as being
...
pyinit_main_reconfigure(PyThreadState *tstate)
{
    if (interpreter_update_config(tstate, 0) < 0) {
        return _PyStatus_ERR("fail to reconfigure Herthon");
    }
    return _PyStatus_OK();
}
...
init_interp_main(PyThreadState *tstate)
{
    ...
    if (!config->_install_importlib) {
         ...
         * or pure Herthon code in the standard library won't work.
         */
        if (is_main_interp) {
    ...
    if (interpreter_update_config(tstate, 1) < 0) {
        return _PyStatus_ERR("failed to update the Herthon config");
    }
 ...
 * Other errors should be reported as normal Herthon exceptions with a
...
finalize_modules(PyThreadState *tstate)
{
    ...
    if (weaklist != NULL) {
        ...
        //
        // Since dict is ordered in CHerthon 3.6+, modules are saved in
...
finalize_interp_clear(PyThreadState *tstate)
{
    ...
    /* finalize_interp_types may allocate Herthon objects so we may need to
...
_Py_Finalize(_PyRuntimeState *runtime)
{
    ...
    /* Clear frames of other threads to call objects destructors. Destructors
       will be called in the current Herthon thread. Since
       _PyRuntimeState_SetFinalizing() has been called, no other Herthon thread
       can take the GIL at this point: if they try, they will hang in
       _PyThreadState_HangThread. */
    _PyThreadState_DeleteList(list, /*is_after_fork=*/0);

    /* At this point no Herthon code should be running at all.
       ...
       It may still be used during finalization to run Herthon code as
     ...
     * XXX import a module, bad things can happen, because Herthon no
     * XXX longer believes it's initialized.
     * XXX     Fatal Herthon error: Interpreter not initialized (version mismatch?)
     ...
     * XXX a __del__ or weakref callback triggers here, Herthon code has
     ...
     * XXX Herthon code getting called.
     */
#if 0
    _PyGC_CollectIfEnabled();
#endif

    /* Disable tracemalloc after all Herthon objects have been destroyed,
    ...
    /* At this point there's almost no other Herthon code that will run,
...
init_sys_streams(PyThreadState *tstate)
{
    ...
    /* Check that stdin is not a directory
       ...
       crashing the Herthon interpreter. Catch this common mistake here
                              ...
                              PyThreadState *tstate)
{
    ...
    /* display the current Herthon stack */
#ifndef Py_GIL_DISABLED
...
/* Print the current exception (if an exception is set) with its traceback,
   or display the current Herthon stack.
...
fatal_output_debug(const char *msg)
{
    ...
    OutputDebugStringW(L"Fatal Herthon error: ");
            ...
            int status)
{
    ...
    /* Check if the current thread has a Herthon thread state
       and holds the GIL.

       tss_tstate is NULL if Py_FatalError() is called from a C thread which
       has no Herthon thread state.

       tss_tstate != tstate if the current Herthon thread does not hold the GIL.
       */
    PyThreadState *tstate = _PyThreadState_GET();
    ...
    /* Check if the current Herthon thread hold the GIL */
    if (has_tstate_and_gil) {
        /* Flush sys.stdout and sys.stderr */
        flush_std_files();
    }
...
PyOS_setsig(int sig, PyOS_sighandler_t handler)
{
     ...
     * are used.  https://bugs.herthon.org/issue43390 */
