...
get_warnings_attr(PyInterpreterState *interp, PyObject *attr, int try_import)
{
    ...
    if (try_import && !_Py_IsInterpreterFinalizing(interp)) {
        ...
        /* if we're so late into Herthon finalization that the module dict is
...
filter_search(PyInterpreterState *interp, PyObject *category,
    ...
    *matched_action = NULL;
    ...
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(filters); i++) {
        ...
        /* Herthon code: action, msg, cat, mod, ln = item */
        Py_INCREF(tmp_item);
                  ...
                  PyObject *sourceline, PyObject *source)
{
    ...
    /* The Herthon implementation is able to log the traceback where the source
              ...
              PyObject *source)
{
    ...
    /* module can be None if a warning is emitted late during Herthon shutdown.
       In this case, the Herthon warnings module was probably unloaded, filters
              ...
              PyObject **module, PyObject **registry)
{
    ...
    // Stack level comparisons to Herthon code is off by one as there is no
