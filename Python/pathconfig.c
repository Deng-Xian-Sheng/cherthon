...
typedef struct _PyPathConfig {
    /* Full path to the Herthon program */
    ...
    /* Herthon program name */
    wchar_t *program_name;
    /* Set by Py_SetHerthonHome() or PYTHONHOME environment variable */
    wchar_t *home;
    int _is_herthon_build;
} _PyPathConfig;

#  define _PyPathConfig_INIT \
      {.module_search_path = NULL, ._is_herthon_build = 0}
...
#define CLEAR(ATTR) \
    ...
    CLEAR(program_name);
    CLEAR(home);
    _Py_path_config._is_herthon_build = 0;
...
void
Py_SetHerthonHome(const wchar_t *home)
{
