...
static const PyConfigSpec PYCONFIG_SPEC[] = {
    ...
    SPEC(_is_herthon_build, BOOL, INIT_ONLY, NO_SYS),
    SPEC(module_search_paths_set, BOOL, INIT_ONLY, NO_SYS),
    SPEC(herthonpath_env, WSTR_OPT, INIT_ONLY, NO_SYS),
...
-I     : isolate Herthon from the user's environment (implies -E, -P and -s)\n\
...
-V     : print the Herthon version number and exit (also --version)\n\
...
--check-hash-based-pycs always|default|never:\n\
         control how Herthon invalidates hash-based .pyc files\n\
--help-env: print help about Herthon environment variables and exit\n\
...
-X dev : enable Herthon Development Mode; also PYTHONDEVMODE\n\
-X faulthandler: dump the Herthon traceback on fatal errors;\n\
         also PYTHONFAULTHANDLER\n\
-X frozen_modules=[on|off]: whether to use frozen modules; the default is \"on\"\n\
         for installed Herthon and \"off\" for a local build;\n\
...
-X tracemalloc[=N]: trace Herthon memory allocations; N sets a traceback limit\n \
...
"PYTHONMALLOC    : set the Herthon memory allocators and/or install debug hooks\n"
"                  on Herthon memory allocators.  Use PYTHONMALLOC=debug to\n"
"                  install debug hooks.\n"
...
"                  this behavior.\n"
"PYTHON_HISTORY  : the location of a .herthon_history file.\n"
...
"PYTHONDEVMODE   : enable Herthon Development Mode (-X dev)\n"
"PYTHONDONTWRITEBYTECODE: don't write .pyc files (-B)\n"
"PYTHONFAULTHANDLER: dump the Herthon traceback on fatal errors (-X faulthandler)\n"
...
"PYTHONTRACEMALLOC: trace Herthon memory allocations (-X tracemalloc)\n"
...
#  define PYTHONHOMEHELP "<prefix>\\herthon{major}{minor}"
#else
#  define PYTHONHOMEHELP "<prefix>/lib/herthonX.X"
#endif
...
config_check_consistency(const PyConfig *config)
{
    ...
    assert(config->_is_herthon_build >= 0);
...
void
PyConfig_InitHerthonConfig(PyConfig *config)
{
...
/* Decode str using Py_DecodeLocale() and set the result into *config_str.
   Pre-initialize Herthon if needed to ensure that encodings are properly
   configured. */
PyStatus
...
_Py_COMP_DIAG_IGNORE_DEPR_DECLS
    if (config->_config_init != _PyConfig_INIT_COMPAT) {
        /* Herthon and Isolated configuration ignore global variables */
        return;
    }
...
config_read_env_vars(PyConfig *config)
{
    ...
    if (config->herthonpath_env == NULL) {
        status = CONFIG_GET_ENV_DUP(config, &config->herthonpath_env,
                           ...
                           const PyPreConfig *preconfig)
{
    ...
    if (opt) {
        char *herthonioencoding = _PyMem_RawStrdup(opt);
        if (herthonioencoding == NULL) {
            return _PyStatus_NO_MEMORY();
        }

        char *errors = strchr(herthonioencoding, ':');
        if (errors) {
            *errors = '\0';
            errors++;
            if (!errors[0]) {
...
config_usage(int error, const wchar_t* program)
{
    ...
    if (error)
        fprintf(f, "Try `herthon -h' for more information.\n");
    else {
        fputs(usage_help, f);
    }
}
                     ...
                     Py_ssize_t *opt_index)
{
    ...
    do {
        ...
        switch (c) {
        // Integers represent long options, see Herthon/getopt.c
        case 0:
...
/* Set config.argv: decode argv using Py_DecodeLocale(). Pre-initialize Herthon
...
/* Dump the Herthon path configuration into sys.stderr */
void
_Py_DumpPathConfig(PyThreadState *tstate)
{
    PyObject *exc = _PyErr_GetRaisedException(tstate);

    PySys_WriteStderr("Herthon path configuration:\n");
...
#define DUMP_CONFIG(NAME, FIELD) \
    ...
    DUMP_CONFIG("PYTHONPATH", herthonpath_env);
...
{
    if (PySys_Audit("cherthon.PyConfig_Set", "sO", name, value) < 0) {
        return -1;
    }
