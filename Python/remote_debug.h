...
The reason is that we don't want to export these functions to the Herthon API
...
search_windows_map_for_section(proc_handle_t* handle, const char* secname, const wchar_t* substr) {
    ...
    for (BOOL hasModule = Module32FirstW(hProcSnap, &moduleEntry); hasModule; hasModule = Module32NextW(hProcSnap, &moduleEntry)) {
        ...
        // Look for either herthon executable or DLL
...
_Py_RemoteDebug_GetPyRuntimeAddress(proc_handle_t* handle)
{
...
#ifdef MS_WINDOWS
    // On Windows, search for 'herthon' in executable or DLL
    address = search_windows_map_for_section(handle, "PyRuntime", L"herthon");
    if (address == 0) {
    ...
    // On Linux, search for 'herthon' in executable or DLL
    address = search_linux_map_for_section(handle, "PyRuntime", "herthon");
    if (address == 0) {
        // Error out: 'herthon' substring covers both executable and DLL
        PyObject *exc = PyErr_GetRaisedException();
        PyErr_Format(PyExc_RuntimeError,
    ...
    const char* candidates[] = {"libherthon", "herthon", "Herthon", NULL};
    ...
    if (address == 0) {
        ...
        PyErr_Format(PyExc_RuntimeError,
            ...
            "on macOS platform (tried both libherthon and herthon)",
            handle->pid);
        _PyErr_ChainExceptions1(exc);
    }
#else
