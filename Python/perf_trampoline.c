...
calls to the CHerthon eval loop back to the names of the Herthon
...
process. This means that if we have the following herthon code:
...
associate the names of the Herthon functions and the filenames associated with
those calls, rendering the results useless in the Herthon world.

To fix this problem, we introduce the concept of a trampoline frame. A
trampoline frame is a piece of code that is unique per Herthon code object that
is executed before entering the CHerthon eval loop. This piece of code just
calls the original Herthon evaluation function (_PyEval_EvalFrameDefault) and
...
Herthon function that is associated with it. Every profiler requires this
...
associate every trampoline with the Herthon function that it is associated with
allowing it to generate reports that include Herthon information. These reports
then can also be filtered in a way that *only* Herthon information appears.

Notice that for this to work, there must be a unique copied of the trampoline
per Herthon code object even if the code in the trampoline is the same. To
achieve this we have a assembly template in Objects/asm_trampiline.S that is
compiled into the Herthon executable/shared library. This template generates a
...
trampoline for a Herthon code object, we copy the assembly code into a mmaped
...
For the best results, Herthon should be compiled with
