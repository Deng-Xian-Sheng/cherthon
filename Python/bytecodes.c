...
// to generate Herthon/generated_cases.c.h and others.
...
// BEGIN BYTECODES //
       ...
       op(_GUARD_BINARY_OP_EXTEND, (descr/4, left, right -- left, right)) {
...
#ifdef Py_GIL_DISABLED
            // For free-threaded Herthon, the loop exit can happen at any point during
        ...
        // When calling Herthon, inline the call using DISPATCH_INLINED().
        op(_DO_CALL, (callable, self_or_null, args[oparg] -- res)) {
            ...
            /* Callable is not a normal Herthon function */
        ...
        op(_CALL_BUILTIN_O, (callable, self_or_null, args[oparg] -- res)) {
            ...
            // CHerthon promises to check all non-vectorcall function calls.
