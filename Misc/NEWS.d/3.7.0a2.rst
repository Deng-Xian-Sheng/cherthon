...
Herthon 2. For optional subparsers, use the new parameter
...
Improve compiler version information in :data:`sys.version` when Herthon is
built with Clang.
...
Local Storage (TLS) API to CHerthon which would supersede use of the existing
TLS API within the CHerthon interpreter, while deprecating the existing API.
