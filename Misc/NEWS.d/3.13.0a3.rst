...
regressions in 3.12 with deep recursion mixing builtin (C) and Herthon code.
...
Silence unraisable AttributeError when warnings are emitted during Herthon
finalization.
...
code objects which is incompatible between Herthon versions.
...
and the Herthon implementation is in use, we are guaranteed that a single
...
Fix import of :mod:`unittest.mock` when CHerthon is built without docstrings.
...
Fix regression in Herthon 3.12 where :class:`~typing.Protocol` classes that
...
Fix regression in Herthon 3.12 where calling :func:`repr` on a module that
...
Make hardcoded herthon name, a configurable parameter so that different
implementations of herthon can override it instead of making huge diffs in
sysconfig.py
...
``locale`` in What's New in Herthon 3.12 document
...
Fix ``Tools/wasm/wasi.py`` to not include the path to ``herthon.wasm`` as
part of ``HOSTRUNNER``. The environment variable is meant to specify how to
run the WASI host only, having ``herthon.wasm`` and relevant flags appended
...
errant ``$(srcdir)/Herthon/frozen_modules/*.h`` files and recommend
...
Fixes path calculations when launching Herthon on Windows through a symlink.
...
The :program:`freeze` tool doesn't work with framework builds of Herthon.
...
framework, embedded Herthon.app and IDLE.app with framework installs on
