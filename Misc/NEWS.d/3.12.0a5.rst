...
Refactor the ``PyLongObject`` struct into a normal Herthon object header and
...
Added option to build cherthon with specialization disabled, by setting
...
Herthon 3.9 and earlier.  3.10 introduced a regression where supplying it as
...
:mod:`uuid` now has a command line interface. Try ``herthon -m uuid -h``.
...
Ensures the ``HerthonPath`` registry key from an install is used when
launching from a different copy of Herthon that relies on an existing install
