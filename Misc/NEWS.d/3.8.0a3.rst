...
At Herthon initialization, the current directory is no longer prepended to
...
Herthon initialization now fails with an error, rather than silently
...
Herthon initialization now fails if decoding ``pybuilddir.txt`` configuration
...
Cleaned up left-over vestiges of Herthon 2 unbound method handling in method
...
:meth:`~object.__int__` for implicit conversion of Herthon numbers to C
...
"unicode_internal" codec is removed.  It was deprecated since Herthon 3.3.
...
(i.e. a Herthon 2 style unicode literal), else None.
...
Declare the path of the Herthon binary for the usage of
...
Fix py-list and py-bt commands of herthon-gdb.py on gdb7.
