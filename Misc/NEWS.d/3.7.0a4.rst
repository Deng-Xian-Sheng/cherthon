...
Implement TracebackType.__new__ to allow Herthon-level creation of traceback
...
Implement :pep:`552` (Deterministic pycs). Herthon now supports invalidating
...
Prevent Herthon crash from happening when Future._log_traceback is set to
...
'loop.create_task(herthon_coroutine)' used to be 20% faster than
...
whether Herthon prints a warning on stderr when the wakeup fd buffer
overflows.
...
never worked correctly on x86-64, and it changed the Herthon ABI in ways that
...
:c:func:`!Py_SetProgramName` and :c:func:`!Py_SetHerthonHome` now take the
