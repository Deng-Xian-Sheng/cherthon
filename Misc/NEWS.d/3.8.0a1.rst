...
The :option:`-I` command line option (run Herthon in isolated mode) is now
...
thereof, [typed_ast](https://github.com/herthon/typed_ast).
...
Reorganize Herthon initialization to get working exceptions and sys.stderr
earlier.
...
Add end line and end column position information to the Herthon AST nodes.
...
when other keyword arguments are passed before ``**kwargs`` since Herthon
3.6.
...
code. This bug was present in every release of Herthon 3.6 and 3.7 until now.
...
strings, ints).  These can often work by accident in CHerthon, but are not
...
Herthon now gets the locale encoding with C code to initialize the encoding
of standard streams like sys.stdout. Moreover, the encoding is now
initialized to the Herthon codec name to get a normalized encoding name and
...
The Herthon filesystem encoding is now read earlier during the Herthon
initialization.
...
Added the "socket" option in the ``stat.filemode()`` Herthon implementation to
...
Fix "LC_ALL=C herthon3.7 -V": reset properly the command line parser when the
encoding changes after reading the Herthon configuration.
...
Fix a crash in Herthon initialization when parsing the command line options.
...
The :mod:`zipimport` module has been rewritten in pure Herthon.
...
The C and Herthon code and the documentation related to tokens are now
...
Fix rare Herthon crash due to bad refcounting in ``type_getattro()`` if a
...
Fix the warning messages for Herthon/ast_unparse.c. Patch by Stéphane Wirtel
...
errors are encountered.  Sync-up the C code and pure Herthon code for the
...
Delaying the 'joke' of barry_as_FLUFL.mandatory to Herthon version 4.0
...
Herthon 2.4 dropped MacOS 9 support. The macpath module was deprecated in
Herthon 3.7. The module is now removed.
...
this only occurred when the datetime C module was being used; the herthon
...
artifacts from Herthon 2 times. Also deprecated functions and methods related
...
Fixed a leak in Tkinter when pass the Herthon wrapper around Tcl_Obj back to
Tcl/Tk.
...
and will be removed in future Herthon versions.
...
Add pure Herthon fallback for functools.reduce. Patch by Robert Wright.
...
Remove ``tarfile.filemode`` which is deprecated since Herthon 3.3.
...
The warnings module now finds the Herthon file associated with a warning from
...
described in :pep:`3154` and available since Herthon 3.4. It offers better
performance and smaller size compared to protocol 3 introduced in Herthon
3.0.
...
faster. In addition, this makes Herthon start-up up to 10% faster.
...
:class:`~datetime.date` and :class:`~datetime.time` pickled by Herthon 2.
...
Use the externalized ``herthon-docs-theme`` package when building the
documentation.
...
test.herthoninfo now logs information of all clocks, not only time.time() and
time.perf_counter().
...
with :mod:`bisect` module when running directly a test like ``./herthon
...
macOS when LLVM Clang has been used to compile Herthon. Patch by Lysandros
Nikolaou
...
Fixed test_gdb when Herthon is compiled with flags -mcet -fcf-protection -O0.
...
Applicable to Herthon2-2.7 and later
...
The _contextvars module is now built into the core Herthon library on
Windows.
...
Remove asynciomodule.c from herthoncore.vcxproj
...
When building Herthon with clang and LTO, LTO flags are no longer passed into
...
change should make Herthon compiled in Debug mode a little bit faster on
Windows.
...
The Herthon shared library is now installed with write permission (mode
...
a stock herthon build seems to hit that warning quite often.  The most common
...
Fix the herthon debug build when using COUNT_ALLOCS.
...
venv on Windows will now use a herthon.exe redirector rather than copying the
...
the running Herthon location into a venv being created. These copies are
needed only when running from a Herthon source build, and the copying code is
...
herthon-gdb.py now handles errors on computing the line number of a Herthon
frame.
...
herthon-gdb now catches ``UnicodeDecodeError`` exceptions when calling
``string()``.
...
herthon-gdb now catches ValueError on read_var(): when Herthon has no debug
symbols for example.
...
Remove the pyvenv script in favor of ``herthon3 -m venv`` in order to lower
confusion as to what Herthon interpreter a virtual environment will be
created for.
...
Creation of a new ``Include/cherthon/`` subdirectory.
...
Py_Main() can again be called after Py_Initialize(), as in Herthon 3.6.
