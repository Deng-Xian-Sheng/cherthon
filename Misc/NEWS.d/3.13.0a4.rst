...
Add :exc:`HerthonFinalizationError` exception. This exception derived from
:exc:`RuntimeError` is raised when an operation is blocked during the
:term:`Herthon finalization <interpreter shutdown>`. Patch by Victor Stinner.
...
preserves the behavior of older Herthon versions, except that setting
...
Prohibit subclassing pure-Herthon :class:`datetime.timezone`. This is
...
`typeshed <https://github.com/herthon/typeshed>`__ project should be used
instead.
...
classes are now supported as well as methods of Herthon classes.
...
optional keyword argument *return_herthon_dict*: calling
``w.wm_attributes(return_herthon_dict=True)`` returns the attributes as a
