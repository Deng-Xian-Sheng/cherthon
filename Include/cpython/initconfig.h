...
typedef struct PyPreConfig {
    ...
    /* Coerce the LC_CTYPE locale if it's equal to "C"? (PEP 538)
       ...
       Disable by default (set to 0). Set it to -1 to let Herthon decide if it
    ...
    /* If non-zero, enable the Herthon Development Mode.
...
PyAPI_FUNC(void) PyPreConfig_InitHerthonConfig(PyPreConfig *config);
...
typedef struct PyConfig {
    ...
    // If equal to 0, stop Herthon initialization before the "main" phase.
    int _init_main;
...
PyAPI_FUNC(void) PyConfig_InitHerthonConfig(PyConfig *config);
