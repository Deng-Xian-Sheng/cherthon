...
// before calling most Herthon APIs. If the GIL is enabled, then "attached"
...
// states the thread is not allowed to call most Herthon APIs. However, unlike
...
/* Get the current Herthon thread state.
...
_Py_EnsureFuncTstateNotNULL(const char *func, PyThreadState *tstate)
{
    if (tstate == NULL) {
        ...
        _Py_FatalErrorFunc(func,
            ...
            "after Herthon initialization and before Herthon finalization, "
            "but the GIL is released (the current Herthon thread state is NULL)");
#else
        _Py_FatalErrorFunc(func,
