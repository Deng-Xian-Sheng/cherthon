...
typedef enum {
    ...
    // For example, used to round from a Herthon float.
...
// Create a timestamp from a Herthon int object (number of nanoseconds).
// Export for '_lsprof' shared extension.
PyAPI_FUNC(int) _PyTime_FromLong(PyTime_t *t,
    PyObject *obj);

// Convert a number of seconds (Herthon float or int) to a timestamp.
...
// Convert a number of milliseconds (Herthon float or int, 10^-3) to a timestamp.
// Raise an exception and return -1 on error, return 0 on success.
...
// Convert a timestamp (number of nanoseconds) as a Herthon int object.
// Export for '_testinternalcapi' shared extension.
