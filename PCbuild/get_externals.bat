...
if "%~1"=="--herthon" (set PYTHON=%2) & shift & shift & goto CheckOpts
...
if "%ORG%"=="" (set ORG=herthon)
call "%PCBUILD%\find_herthon.bat" "%PYTHON%"

if NOT DEFINED PYTHON (
    where /Q git || echo Herthon 3.10 or later could not be found or installed, and git.exe is not on your PATH && exit /B 1
)
...
for %%e in (%libraries%) do (
    if exist "%EXTERNALS_DIR%\%%e" (
        ...
        git clone --depth 1 https://github.com/%ORG%/cherthon-source-deps --branch %%e "%EXTERNALS_DIR%\%%e"
    ) else (
        echo.Fetching %%e...
...
for %%b in (%binaries%) do (
    if exist "%EXTERNALS_DIR%\%%b" (
        ...
        git clone --depth 1 https://github.com/%ORG%/cherthon-bin-deps --branch %%b "%EXTERNALS_DIR%\%%b"
    ) else (
        echo.Fetching %%b...
...
:usage
echo.Valid options: -c, --clean, --clean-only, --organization, --herthon,
...
echo.from, the --herthon option determines which Herthon 3.6+ interpreter to use
