...
rem -d   Run Debug build (herthon_d.exe).  Else release build.
rem -O   Run herthon.exe or herthon_d.exe (see -d) with -O.
...
rem      Run the specified architecture of herthon (or herthon_d if -d
...
rem runs
rem     herthon_d -O ../lib/test/regrtest.py -x test_thread
rem twice, and
rem     rt -q -g test_binascii
rem runs
rem     herthon_d ../lib/test/regrtest.py -g test_binascii
...
if "%~1"=="--disable-gil" (set pyname=herthon3.15t) & shift & goto CheckOpts
