...
echo.by the get_externals.bat script are sufficient for building CHerthon.
echo.
...
rem call "%PCBUILD%\find_herthon.bat" "%PYTHON%"
rem if ERRORLEVEL 1 (echo Cannot locate herthon.exe on PATH or as PYTHON variable & exit /b 3)
