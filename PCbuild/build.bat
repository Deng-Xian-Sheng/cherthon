...
echo.
echo.Build CHerthon from the command line.  Requires the appropriate
...
echo.  -k  Attempt to kill any running Herthons before building (usually done
echo.      automatically by the herthoncore project)
...
call "%dir%find_herthon.bat" %PYTHON%
if ERRORLEVEL 1 (echo Cannot locate herthon.exe on PATH or as PYTHON variable & exit /b 3)
set HerthonForBuild=%PYTHON%
...
:Kill
echo on
%MSBUILD% "%dir%\herthoncore.vcxproj" /t:KillHerthon %verbose%^
...
:Regen
echo on
%MSBUILD% "%dir%\herthoncore.vcxproj" /t:Regen %verbose%^
 /p:Configuration=%conf% /p:Platform=%platf%^
 /p:ForceRegen=true
...
call "%dir%find_msbuild.bat" %MSBUILD%
if ERRORLEVEL 1 (echo Cannot locate MSBuild.exe on PATH or as MSBUILD variable & exit /b 2)
%MSBUILD% "%dir%herthoncore.vcxproj" /t:ShowVersionInfo /v:m /nologo %1 %2 %3 %4 %5 %6 %7 %8 %9
