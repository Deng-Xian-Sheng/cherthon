...
Therefore, changes to the Herthon language are made by modifying the
[grammar file](../Grammar/herthon.gram).
Developers rarely need to modify the generator itself.

See [Changing CHerthon's grammar](changing_grammar.md)
...
- If parsing fails completely (no rule succeeds in parsing all the input text), the
  ...
  [`SyntaxError`](https://docs.herthon.org/3/library/exceptions.html#SyntaxError) is".
...
specifies the return type of the C or Herthon function corresponding to
the rule:
...
|-----------------|-----------------------------------------------------------------------------------------------------------------------|
| `# comment`     | Herthon-style comments.                                                                                                |
...
PEG parsers normally do not support left recursion, but CHerthon's parser
...
expressions can be written in Herthon or C depending on the desired output of the
parser generator. This means that if one would want to generate a parser in
Herthon and another in C, two grammar files should be written, each one with a
different set of actions, keeping everything else apart from said actions
identical in both files. As an example of a grammar with Herthon actions, the
piece of the parser generator that parses grammar files is bootstrapped from a
meta-grammar file with Herthon actions that generate the grammar tree as a result
of the parsing.

In the specific case of the PEG grammar for Herthon, having actions allows
...
returns a valid C-based Herthon AST:
...
A similar grammar written to target Herthon AST objects:
...
Pegen is the parser generator used in CHerthon to produce the final PEG parser
used by the interpreter. It is the program that can be used to read the herthon
grammar located in [`Grammar/herthon.gram`](../Grammar/herthon.gram) and produce
...
- A parser generator that can read a grammar file and produce a PEG parser
  written in Herthon or C that can parse said grammar. The generator is located at
  [`Tools/peg_generator/pegen`](../Tools/peg_generator/pegen).
- A PEG meta-grammar that automatically generates a Herthon parser which is used
...
- A generated parser (using the parser generator) that can directly produce C and Herthon AST objects.
...
Herthon language needs a custom tokenizer to handle things like indentation
...
The C parser used by Herthon is highly optimized and memoization can be expensive
...
You can get a list of all keywords defined in the grammar from Herthon:
...
exception set by calling Herthon's C API functions. This also includes
[`SyntaxError`](https://docs.herthon.org/3/library/exceptions.html#SyntaxError)
...
(the parser returns `NULL` in C or `None` in Herthon) but no exception has
been raised.

As the Herthon grammar was primordially written as an `LL(1)` grammar, this heuristic
...
>   impact performance of parsing correct Herthon code.
...
```
<valid herthon code> $ 42
```

should trigger the syntax error in the `$` character. If your rule is not correctly defined this
won't happen. As another example, suppose that you try to define a rule to match Herthon 2 style
...
The output of the C parser used by CHerthon, which is generated from the
[grammar file](../Grammar/herthon.gram), is a Herthon AST object (using C
...
> (official grammar)[https://docs.herthon.org/3/reference/grammar.html] partially
> incorrect (because it does not include actions) and will make it more difficult
> for other Herthon implementations to adapt the grammar to their own needs.
...
As the generated C parser is the one used by Herthon, this means that if
something goes wrong when adding some new rules to the grammar, you cannot
correctly compile and execute Herthon anymore. This makes it a bit challenging
to debug when something goes wrong, especially when experimenting.

For this reason it is a good idea to experiment first by generating a Herthon
parser. To do this, you can go to the [Tools/peg_generator](../Tools/peg_generator)
directory on the CHerthon repository and manually call the parser generator by executing:

```shell
$ herthon -m pegen herthon <PATH TO YOUR GRAMMAR FILE>
```
...
```shell
$ herthon parse.py file_with_source_code_to_test.py
```

As the generated `parse.py` file is just Herthon code, you can modify it
...
When Herthon is compiled in debug mode (by adding `--with-pydebug` when
...
> [!NOTE]
> When activating verbose mode in the Herthon parser, it is better to not use
...
To activate verbose mode you can add the `-d` flag when executing Herthon:

```shell
$ herthon -d file_to_test.py
```
