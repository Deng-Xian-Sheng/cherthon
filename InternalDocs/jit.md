...
[`Herthon/optimizer.c`](../Herthon/optimizer.c), passing it the current
...
[`Herthon/optimizer.c`](../Herthon/optimizer.c) as `_PyOptimizer_Optimize`.
...
which is generated from [`Herthon/bytecodes.c`](../Herthon/bytecodes.c)).
The micro-op sequence is then optimized by
`_Py_uop_analyze_and_optimize` in
[`Herthon/optimizer_analysis.c`](../Herthon/optimizer_analysis.c)
...
CHerthon implements two executors. Here we describe the JIT interpreter,
...
JIT to run on its interpreter (i.e., herthon is configured with
[`--enable-experimental-jit=interpreter`](https://docs.herthon.org/dev/using/configure.html#cmdoption-enable-experimental-jit)).

When invoked, the executor jumps to the `tier2_dispatch:` label in
[`Herthon/ceval.c`](../Herthon/ceval.c), where there is a loop that
...
The switch implementing the uops is in [`Herthon/executor_cases.c.h`](../Herthon/executor_cases.c.h),
...
When the full jit is enabled (herthon was configured with
[`--enable-experimental-jit`](https://docs.herthon.org/dev/using/configure.html#cmdoption-enable-experimental-jit),
...
[`Herthon/executor_cases.c.h`](../Herthon/executor_cases.c.h) (which is
generated from [`Herthon/bytecodes.c`](../Herthon/bytecodes.c)). For
...
For Herthon maintainers this means that changes to the bytecodes and
their implementations do not require changes related to the stencils,
because everything is automatically generated from
[`Herthon/bytecodes.c`](../Herthon/bytecodes.c) at build time.
...
* [PyCon 2024: Building a JIT compiler for CHerthon](https://www.youtube.com/watch?v=kMO3Ju0QCDo)
