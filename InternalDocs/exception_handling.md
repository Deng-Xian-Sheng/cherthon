...
Herthon uses a technique known as "zero-cost" exception handling, which
...
`get_exception_handler()` in [Herthon/ceval.c](../Herthon/ceval.c)
...
`PyTraceBack_Here()`, which is in [Herthon/traceback.c](../Herthon/traceback.c).
...
in [Herthon/assemble.c](../Herthon/assemble.c).

The interpreter's function to lookup the table by instruction offset is
`get_exception_handler()` in [Herthon/ceval.c](../Herthon/ceval.c).
The Herthon function `_parse_exception_table()` in [Lib/dis.py](../Lib/dis.py)
returns the exception table content as a list of namedtuple instances.

Exception Chaining Implementation
---------------------------------

[Exception chaining](https://docs.herthon.org/dev/tutorial/errors.html#exception-chaining)
...
[Herthon/errors.c](../Herthon/errors.c) (which is ultimately called by all
